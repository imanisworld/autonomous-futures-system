...
def process_alert(
    ...
    if five_min_enabled() and is_five_min(payload.timeframe):
        ...
        if not five_min_trigger and (
            ...
            or "strat_12hr_miyagi" in cfg.enabled_concepts
        ):
            # Unlike legacy concepts, the resolved 4HR Re-Trigger, 3-2-2
            # First Live, and 12HR Miyagi setups are established and
            # triggered by the authoritative 5-minute stream itself (see
            # signal_engine.py's _FIVE_MINUTE_NATIVE_STRATEGIES).
    ...
    if daily_state.has_open_position:
        ...
        # but it used to early-return before any log_decision, so a blocked bar
        # left NO journal record (the 2026-07-22 orphan blinded the pipeline for a
        # whole session and the journal just showed zero decisions). Journal an
        # INERT BLOCK_VISIBILITY record classifying WHY evaluation did not run.
        # Broker state is not read here (no hot-path I/O); the reconciler remains
        # the broker-authoritative drift detector. Fail-soft.
        try:
        ...
        # Context-feed: also preserve the observe-only market-context row that the
        # post-decision path would otherwise skip on this early return. Distinct
        # data from the visibility record above (context evidence vs block reason);
        # both are fail-soft and neither affects the decision/risk/broker path.
        _observe_strategy_context_once(
            _capacity_observation(
