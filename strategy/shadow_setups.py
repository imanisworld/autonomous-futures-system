...
from strategy.transition_failed_breakdown_reclaim import detect_transition_failed_breakdown_reclaim
...
def _bars_with_current(state: MarketState, bars: list[dict]) -> list[dict]:
    """Return the recent bars with the current bar appended when needed."""
...
def _failed_breakdown_reclaim(
    ...
    """Observe the shared failed-breakdown reclaim signal without execution."""
    ...
    return _candidate(
        ...
        entry=signal.entry,
        stop=signal.reference_stop,
        target=signal.reference_target,
        notes=f"Shadow: {signal.notes}",
    )
