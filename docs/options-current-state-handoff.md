...
**ADVISORY / READ-ONLY OPTIONS POSTURE; EVIDENCE COLLECTION ACTIVE; STRATEGY EDGE NOT PROVEN; 2026-09-24 DEPLOYMENT NOT RE-VERIFIED.**

Latest box-verified scanner identity recorded in this file is the 2026-09-23 read-only observation of release `5b7be0f7`; that runtime was **not** re-verified by this 2026-09-24 repository refresh. Older release identities below are retained as dated provenance only.
...
**Cohort `V1-EPOCH-3` (`DAILY_SCORER_ALIGNMENT`) started 2026-09-21T19:23:01Z** on the first fully clean (20/20, zero `market_data_error`) scheduled cycle on options-scanner release `aea0df0` (= `75b2687` + #888). PR #869 changed the scorer so Daily (`1D`) setups score on SPY/QQQ daily-trend alignment instead of the 30-minute VWAP/EMA20 checks, which had been vetoing 30 of 64 Daily rows that reached the Discord decision since 2026-09-08 on sub-0.1% intraday wobbles; 30-minute scoring is unchanged and `OPTIONS_PAPER_V1` (`paper_v1.py`) never reads the score, so alert eligibility is the only thing that changed. Between the scorer landing (release `dd3aa9d`, 2026-09-21T04:21:21Z) and epoch-3's start, a pre-existing `SignaContextStore` connection leak (#802, exposed once #870's dashboard began polling `/shadow-journal`/`/status` every minute) saturated the scanner's fd ulimit at 07:55:46Z: **zero scans were recorded 2026-09-21T07:55Z-19:18Z.** PR #888 closed the leak (verified fd-flat over 50 synthetic polls and 6 real scheduled cycles); release `aea0df0` deployed 19:18:07Z is the epoch's `deployed_sha`. Futures-bot was not restarted for any of this. Full timeline, cohort-boundary ids, and outage details: `docs/options_v1_evidence_epoch.json`.

The options scanner has its **own service-specific immutable release**, independent of the futures bot. **Historical 2026-09-20 baseline:** the scanner was pinned to `a0c34818faaad37b20d8c05249e8f5442d8d7141`, a curated release built from the previously proven scanner release `c7798d4993d1ecfd872313cfc5c84da2cda6625d` plus the merged #824/#825 RTH alert-delivery safety changes only. Later dated operational refresh sections supersede this release identity. The futures bot remains on `c7798d4993d1ecfd872313cfc5c84da2cda6625d` and was not restarted. Post-promotion proof: options health remained healthy/advisory-only, Public stayed read-only, `order_supported=false`, account endpoints remained forbidden, production SQLite remained `/root/afs-shared/logs/options_scanner.sqlite`, scheduler remained running, `signa_context_pull_enabled=true`, release integrity passed **1,394/1,394 files**, and the pre/post production DB counts were unchanged (`scans=29,873`, `options_shadow_journal=9,362`, `options_contract_marks=2,511`, `options_selector_evidence=29`). A no-network delivery-boundary proof on the deployed release confirmed **15:59 ET scan -> 16:01 ET delivery = `market_closed`, no HTTP request**. The service-aware drift gate accepts the split release state: futures `c7798d4993d1` integrity PASS, options `a0c34818faaa` integrity PASS; `main` differences are INFO only. The earlier same-day Signa and operator-message promotions remain historical provenance; the first natural Monday RTH scheduled Signa-cycle proof is still pending.
...
#772 closes a provenance ambiguity in that trigger-context work: the context cutoff is the **start of the first crossing 5-minute bucket**; the completed-alignment view uses completed 30-minute bars plus the prior completed Daily candle; the legacy `developing_*` view is only a **completed-30m-derived HTF proxy that excludes the trigger-containing 30-minute bar**, not exact partial live hourly/Daily state. This is an evidence-labeling correction only; it does not change a strategy rule or authorize a new market-context policy.
...
- **1-2-2 causal-source follow-up:** the preregistered 186-arm audit supports IEX only as a distinct miss-allowed research clock with delayed SIP reconciliation: 73 SIP reversals, 71 IEX provisionals, **69/71 (97.18%)** confirmed, **4/73 (5.48%)** SIP reversals missed, **2/71 (2.82%)** provisionals rejected, 0 blocked, 0 negative-latency. The next step is now active: epoch `122-IEX-E1` is preregistered at **60s cadence / 120s IEX->selector capture limit / 16m delayed-SIP reconciliation**, and the observation-only collector is deployed/scheduled on immutable release `36e73f1981850b66b043d849ce877c15bd1ab3e7`. It is isolated from scanner/risk/broker/order state and has not changed V1. Strategy stop/target/runner remain unresolved. First natural RTH acceptance proof is still pending. See `docs/options-122-iex-provisional-results-2026-09-18.md`, `docs/options-122-prospective-collector-preregistration-2026-09-18.md`, and `docs/options-122-prospective-deployment-2026-09-18.md`.
...
- The options scanner is now pinned to immutable release `a0c34818faaad37b20d8c05249e8f5442d8d7141`. This preserves the conservative Signa scheduler and adds only the #824/#825 RTH alert-delivery guard on top of the previously proven `c7798d4993d1ecfd872313cfc5c84da2cda6625d` scanner baseline. Post-restart release integrity passed **1,394/1,394 files**; health remained advisory-only/read-only and the production DB counts were preserved. The earlier forced Signa pull proof remains valid; first natural Monday RTH scheduled-cycle proof is still pending.
...
- A second, deeper readiness audit on 2026-09-20 passed **229 focused options/runtime tests** covering scanner lifecycle/preflight/Public integration, V1 policy + late-entry guards, selector evidence, storage/restart behavior, aggregate risk, broker/endpoint safety, and the 1-2-2 collector.
- The deployed 1-2-2 systemd service was then invoked once during the closed Sunday session after the reboot. It exited `CLOSED_SESSION`, `Result=success`, `ExecMainStatus=0`, wrote **zero** resolutions/triggers, and created **no** prospective journal or raw-trade evidence. The timer's Monday 09:00 ET next firing remained unchanged.
- Live V1 OPEN ACTIVE risk reconciles exactly: WMT **$140.00** + NVDA **$230.00** + XOM **$91.25** = **$461.25**, matching `/status`; all are below the $300 per-trade cap and the total is below the $1,000 aggregate cap. Scheduled resolver code is explicitly fail-closed outside NYSE market hours, so the weekend does not resolve OPEN rows from stale quotes.
- Diff review against the pinned releases found **no undeployed collector fix**: the active 1-2-2 collector/service files are byte-equivalent to `main` since release `36e73f1`. On the V1 scanner active path, `app.py`, `scanner_legacy.py`, `paper_v1.py`, `market_data.py`, and `alert_ranker/config.py` have no pending runtime change relative to the pinned scanner release; the observed `storage.py` difference is only DDL creation-order movement for the already-present episode-block table. No scanner bug fix is sitting on `main` waiting for deployment.
- The NYSE session calendar resolves Monday 2026-09-21 as a normal **13:30-20:00 UTC / 09:30-16:00 ET** session, not an early close.
...
1. **Running V1 scanner:** selector-evidence/replay now runs on scanner release `a0c34818faaad37b20d8c05249e8f5442d8d7141`. The latest release change is alert-delivery safety only; V1 strategy/risk/selector policy is unchanged. Continue natural V1 collection without tuning. Do not treat Signa context rows as trade authority. The next Signa-specific gate is first natural Monday RTH scheduled-pull proof.
...
3. Preserve the first prospective classification: 2-1-2 reversal = `NO LONGER SHOWING EXCESS`; 1-2-2 = `PERSISTING POSSIBLE SIGNAL`. The 1-2-2 source study and collector preregistration/build/deployment are complete. The next 1-2-2 gate is **natural forward evidence on the frozen `122-IEX-E1` release**, beginning with the first accepted RTH causal option-capture + delayed-SIP reconciliation row. Do not expand V1 or tune the epoch automatically.
...
- **DEPLOYED / SMOKE PROVEN at 2026-09-22T03:26:42Z.** Curated reporter pin `b60931a6a9f8-reporter-6512dc3578e9` uses the exact prior `b60931a6a9f8839ab9bbadb88eb6c5cb93cccd28` pin with only `scripts/paper_collection_report.py` overlaid from #899 (`6512dc...`). The three `ops/` dependencies are byte-identical to the prior pin; this is not a pure full-tree `6512dc` release.
- Four-file/hash checks passed. Old and curated `--no-discord` runs on an identical captured real EOD input matched all non-presentation artifact data and the futures card. Invalid/unscoped table states hid raw counts. One Options Discord smoke was posted and API read-back verified its rendered-field payload; visual client inspection was not completed.
...
## 2026-09-23 — 1-2-2 collector fix proven, Signa recovered

Read-only box checks at 13:05, 13:20, 13:45 and 22:27 UTC. Nothing was changed.

- **Collector `db9bc7e2`: FIX PROVEN.** The 13:00 UTC first run exited 0 in 6.18s with status `OFF_RTH_RECONCILIATION_ONLY`. `journal_setup_fingerprint_drift_58` did not recur. The sha256 of the first 58 journal lines matches the pre-fix baseline. 0 failed runs from 13:00 to 22:27 UTC.
- **Delayed SIP backlog cleared:** all 5 pending setups reconciled `PROVEN` at 13:00:01 UTC (4 `CONFIRMED_SAME_REVERSAL`, 1 `MISS_NO_PROVISIONAL`). `690ee090` has no rows after its 09-22 `SOURCE_DRIFT` and stays a terminal drifted block.
- **RTH collection resumed:** `RTH_COLLECTION` cycles took about 16–25s, `cycle_over_cadence` was false, and 0 `data_blocked` / `COLLECTOR_ERROR`. First new setup: TLT `8c74477e` (30m, short). It armed and reversed on the 13:30 bar. Option evidence was `DATA_BLOCKED` / `no_proven_pretrigger_arm`, as expected when a setup arms and triggers in the same cycle. It later reconciled `CONFIRMED_SAME_REVERSAL` / `PROVEN`.
- **Signa: recovered.** Fresh snapshots from 13:30:52 UTC: 29 OK, 1 ReadTimeout on `api/v1/scan` (13:40:44 UTC), 0 × 429. Fresh vs blocked telemetry is truthful. A natural backoff expiry was **not** proven: the options scanner was restarted manually at 06:55:43 UTC on the same release `5b7be0f7` (pid 2362174, from a root SSH session), which reset the in-memory backoff. The restart is unexplained in the handoff log.
- **Still open:** the collector needs many more RTH sessions before it counts as evidence. Watch for repeated Signa ReadTimeouts. The `zz-rollback-protect-20260922.conf` drop-in is still present.
