...
The options scanner has its **own service-specific immutable release**, independent of the futures bot. Current options-scanner production release after the 2026-09-20 Signa runtime update is **`1fc0ad9c97d6daca02905058b14123135605c769`**. This is an important runtime change: the scanner had still been pinned to old immutable release `58f1c50583d8bb747c0b221eabb75af376b10ecc`, so enabling scheduled Signa context pulls was **not** just an env-flag flip. It required building/promoting a new options-scanner release, changing only the options-scanner service pin, and restarting `options-scanner.service`. Futures-bot was not restarted. Post-restart proof showed healthy advisory-only operation, Public read-only provider, `order_supported=false`, account endpoints forbidden, unchanged production SQLite path `/root/afs-shared/logs/options_scanner.sqlite`, scheduler running, `signa_context_pull_enabled=true`, and no broker/order activity. The scheduled Signa pull remains context-only and conservative by default; first natural Monday RTH scheduled-cycle proof is still pending.

Repository `main` now also includes later presentation/observability changes through **#812** and **#815**: standardized Discord/operator message wording and standardized options-scanner alert card rendering. Those changes are merged and CI/CodeQL green, but they are **not proven deployed to the running options-scanner service** in this handoff. As of this note, the running options-scanner production pin remains `1fc0ad9c97d6daca02905058b14123135605c769`; deploying #812/#815 requires a service-specific options-scanner release build, systemd pin update, and `options-scanner.service` restart with the same advisory-only/no-order proof. Futures-bot must not be restarted for that presentation-only options deployment.
