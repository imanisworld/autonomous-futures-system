...
- **1-2-2 causal-source follow-up:** a separately preregistered outcome-independent audit now tests all **186** structurally armed 1-2-2 watch windows on the frozen primary-20 SIP snapshot against IEX-first / delayed-SIP reconciliation. SIP produced 73 authoritative reversals; IEX emitted 71 provisional reversals; **69/71 (97.18%)** confirmed, **4/73 (5.48%)** SIP reversals were missed, and **2/71 (2.82%)** provisionals were rejected because SIP continuation broke first. There were **0 blocked** and **0 negative-latency** rows. Verdict: `MISS_ALLOWED_RESEARCH_OBSERVER_FEASIBLE`, **not SIP-equivalent**. This creates a technically feasible path to prospective causal 1-2-2 observation; it does not authorize V1 expansion, collector deployment, target selection, DEMO, or live trading. See `docs/options-122-iex-provisional-results-2026-09-18.md`.
...
3. Preserve the first prospective classification: 2-1-2 reversal = `NO LONGER SHOWING EXCESS`; 1-2-2 = `PERSISTING POSSIBLE SIGNAL`. The 1-2-2 IEX/delayed-SIP source study is now complete and supports only a separate miss-allowed causal research observer. The next 1-2-2 gate is to pre-register capture lag, cadence, causal endpoint, and observation-only isolation before any prospective collection; do not expand V1 automatically.
