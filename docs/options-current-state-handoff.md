...
The options scanner has its **own service-specific immutable release**, independent of the futures bot. Current options-scanner production release after the 2026-09-20 operator-message deployment is **`c7798d4993d1ecfd872313cfc5c84da2cda6625d`**. The earlier same-day Signa runtime update from old immutable release `58f1c50583d8bb747c0b221eabb75af376b10ecc` to `1fc0ad9c97d6daca02905058b14123135605c769` proved that enabling scheduled Signa context pulls was **not** just an env-flag flip: it required a service-specific options-scanner release pin and an `options-scanner.service` restart. The later `c7798d4` promotion is presentation/read-only/reporting work: standardized Discord operator cards, health/read-only report formatting, a read-only Signa storage report, and public terms/privacy wording. Post-restart proof showed healthy advisory-only operation, Public read-only provider, `order_supported=false`, account endpoints forbidden, unchanged production SQLite path `/root/afs-shared/logs/options_scanner.sqlite`, scheduler running, `signa_context_pull_enabled=true`, release integrity passing for 1,394 files, and no broker/order activity. The scheduled Signa pull remains context-only and conservative by default; first natural Monday RTH scheduled-cycle proof is still pending.
...
- The options scanner is now pinned to immutable release `1fc0ad9c97d6daca02905058b14123135605c769` for the conservative Signa context scheduler. Release verification passed **1,390/1,390 files**. Health proof after restart: scheduler running, `signa_context_pull_enabled=true`, Public read-only provider, `order_supported=false`, account endpoints forbidden, production SQLite path unchanged, and futures-bot not restarted. Because the enablement happened outside RTH, the scheduled job correctly returned `market_closed`; a forced conservative pull succeeded with 13 endpoint attempts, 0 failures, 17 rows stored, `observation_only=true`, and `trade_authority=false`. First natural Monday RTH scheduled-cycle proof remains pending.
...
**Market Hours v2 / Extended Equity Session Readiness:** the first infrastructure slice is now built and tested on `fix/options-alert-market-hours`: a centralized US-equity RTH session authority is shared by scheduled scanner gating and the final Discord send boundary, so manual/webhook alerts cannot bypass weekend/holiday/pre-market/post-close/early-close suppression. Focused proof is 56 passed; the full tracked project suite is 6,383 passed / 7 skipped. This is **not deployed** and does not authorize extended-hours alerts. The remaining extended-session work stays backlog/observation-only: preserve RTH as default authority, isolate any overnight/pre-market/after-hours cohorts, and require provider/liquidity/slippage proof before those sessions can affect alerts, scoring, risk, or entries. See `docs/options-market-hours-alert-gate-2026-09-20.md`.
