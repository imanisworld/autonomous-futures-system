...
Repository `main` now includes the options research/evidence chain through the controlled target/horizon work in **#747/#750**. #710/#712/#715 production-selector evidence/replay is deployed only through the curated scanner release above; unrelated `main` changes, including the dedicated 212R collector, its recent-SIP entitlement preflight, and the later research controls, are **not** deployed to the scanner.
