...
- Scan the bars **in chronological order**, tracking whether a break has occurred yet:
  - For CALLS: a break occurs at the first bar whose **HIGH** exceeds the 4AM high
    ...
    above"). Once a break has occurred, use the first bar whose **CLOSE** is below the
    4AM high — this confirms the retrace. The break bar itself qualifies when its high
    breaks the level and its final close finishes back below it; a later bar is not required.
  - For PUTS: symmetric — a break occurs at the first bar whose **LOW** is below the 4AM
    low; the retrace is the first bar at or after that break whose **CLOSE** is above the
    4AM low. The break bar itself qualifies when it closes back above the level.
...
- If a break occurs but no qualifying bar closes back through the level at or after it, before
...
- No break ever occurs within the window (developing high/low never exceeds the 4AM
  level) → return `None`
- A break occurs but no bar at or after the break closes back through the level before 9:30 → return
  `None`
...
return {
    ...
    "entry_price": ten_am_open if gap_open else entry_trigger,
    "entry_bar_ts": ten_am_bar_ts,
    "gap_open": bool,
