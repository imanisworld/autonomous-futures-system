...
*Evidence classifications reconciled through 2026-09-18; edge-decomposition audit verdicts applied 2026-09-07 with the 4HR trigger-timing correction below.*
...
| 4HR Re-Trigger (MNQ) | ✅ — retrigger is a level touch, no 5m close required | ✅ detector; 1m evidence lane deployed | ⚠️ full-engine/risk parity audited; trigger-time parity corrected offline 2026-09-18, prospective 1m proof pending | ✅ pre-armed stop-touch: 80/81 fills, +$1,414.60 at 1 tick; old +$2,886.60 trigger-backfill headline retired as executable evidence; completed-5m IOC8 only 37 fills / +$1,266.24 | ✅ pre-armed 3-tick H1 +$802.80 / H2 +$491.80 | ✅ pre-armed +$1,294.60 at 3 ticks | n=81 / 80 pre-armed fills; forward causal-1m sample still pending | historical `RISK_GATES_REMOVE_EDGE` still applies to the real-account cap/R:R; **2026-09-18 timing defect also proven** | **PROMISING BUT UNPROVEN / PAPER ONLY — positive after timing correction; not executable under current real-account risk** |
...
| 12HR Miyagi | ✅ | ⚠️ research detector only (`research/detector_12hr_miyagi.py`); `strat_12hr_miyagi` is not wired into `signal_engine.py` | ⚠️ completed-hour lookahead repaired (#776), but 2026-09-19 causal audit found trigger-touch bar stop/T1 suppression in replay; no full-engine result possible | ⚠️ causal trigger-bar A/B at 2 ticks: MNQ 8 fills, 6W/2L, **+$425.33 / PF 2.322**; MES unchanged +$138.85 / PF 1.593 | ⚠️ MNQ H1 +$291.82 / H2 +$133.51, but H2 has only 1 fill; MES H2 −$1.84 at 2 ticks | ✅ causal MNQ remains positive through 4 ticks (+$409.33 / PF 2.257); MES H2 negative at 2–4 ticks | ❌ MNQ n=8, MES n=10 — far below any cell minimum | MNQ remains `RISK_GATES_REMOVE_EDGE` under current account caps; MES remains weak; **replay identity must be fixed/regenerated before trusting the old headline** | **PROMISING BUT UNPROVEN / PARKED / REPLAY DEFECT FOUND; also BROKEN FOR CURRENT SYSTEM RISK CONSTRAINTS** |
...
| MES 1-2-2 (`strat_122`) | ✅ | ✅ | ⚠️ canonical state/replay math is causal, but **operational broker parity is absent**: no order is actually armed before the watched 15m bar; late non-Paper submission fails closed | ✅ reconstructed gap-aware / pessimistic same-bar paper fills | ⚠️ executable subset thin | ✅ historical stress, but stronger slippage weakens/turns edge negative | 16/33 canonical candidates executable | not decomposed | **PROMISING BUT UNPROVEN / PAPER EVIDENCE ONLY / EXECUTION-PARITY GAP** |
...
| ~~**Miyagi completed-hour timing audit**~~ — **DONE by #776**, but **follow-up causal audit 2026-09-19 found a second replay defect** (`docs/futures-causal-mechanics-audit-2026-09-19.md`): the trigger-touch 5m bar is excluded from immediate stop/T1 resolution. Frozen MNQ 2024-09-18 flips WIN→LOSS; 2-tick MNQ becomes +$425.33 / PF 2.322. Research replay correction/regeneration is now required; runtime remains parked. | replay identity | research-only fix; no deploy |
