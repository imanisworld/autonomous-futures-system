...
*Evidence classifications reconciled through 2026-09-22; edge-decomposition audit verdicts applied 2026-09-07 with the 4HR trigger-timing correction and #798/#799/#807 observation lanes below. 12HR Miyagi evidence verdict set to WAIT on 2026-09-24 by Operator ruling AFS-0063. The prior repo label was PROMISING BUT UNPROVEN. See the Miyagi profile.*
...
> 2. **Armed-trigger day strategies** (4HR MNQ, 3-2-2, Miyagi MNQ): the 2026-09-07 standardized waterfall classified them as `RISK_GATES_REMOVE_EDGE`, and the real-account stop/R:R constraints still bind. **4HR and 3-2-2 both received trigger-timing corrections on 2026-09-18.** 4HR: same 81 candidates, old trigger backfill +$2,886.60 / 80 fills, completed-5m IOC8 +$1,266.24 / 37 fills, causal pre-armed touch +$1,414.60 at 1 tick and +$1,294.60 at 3 ticks with both halves positive. 3-2-2: same frozen 34 candidates, completed-close IOC32 +$1,838.40 at 3 ticks / 20 fills, causal pre-armed First Live **+$2,709.66 / 33 fills**, H1 +$1,366.34 / H2 +$1,343.32, one bracket-invalid no-fill. Both corrected timing studies preserve positive historical evidence, but neither removes current real-account risk incompatibility or the need for prospective proof. Miyagi's evidence verdict is **WAIT** (Operator ruling AFS-0063). It remains a research detector, not executable or promotable under current account and runtime constraints, and nothing is activated. See the Miyagi profile.
...
| ORB Breakout — inverted (MNQ evidence lane) | ✅ | ✅ | ✅ | ❌ under the decision-time reference through the production PaperBroker (8-tick IOC, 1 adverse tick, pessimistic, #508 guard): **41 of 63 arms are `ENTRY_BRACKET_INVALID_AT_FILL`**; the 2026-09-07 "57 fills" used a fill one 5m bar late and held 37 fills beyond their own stop | ❌ admissible 22 fills: H1 +$75.18 / **H2 −$45.74** (2026-09-08) | n/a — nothing to stress | ❌ 22 admissible fills, +$29.44, PF 1.14 (`docs/inverse-orb-decision-time-replay-2026-09-08.md`); the +$1,026.64 / PF 5.28 (63 arms) and +$745.72 / PF 2.39 (n=111, 66/111 invalid) baselines are **RETIRED** | `BRACKET_DESTROYS_EDGE` — the mirrored 50-tick stop sits inside the breakout bar's own range (median favourable detachment 68 ticks at the decision close); the gated "positive" was the detached-fill artifact | **RETIRED / NOT ACTIVE** (#517, 2026-09-08); inverse env pins are accounting isolation only, not activation; no promotion authority | **BROKEN — negative evidence** (downgraded 2026-09-08) |
| MES 1-2-2 (`strat_122`) | ✅ | ✅ | ⚠️ 2026-09-19 pre-arm audit: final 15m arm-bar type/high/low are known only at bar close, exactly when the watched bar opens; **exact pre-open execution parity is not feasible as the same strategy identity**. Late non-Paper submission correctly fails closed. | ❌ 2026-09-23: booked entry is the trigger price inside the watched bar, but the decision (gate labels) is only final at that bar's close; entry at decision-close +1 tick turns the #547 year +$26 → **−$465** | ❌ older independent year (2024-09-23 → 2025-07-23): 28 trades, −$89, PF 0.84 even with booked fills; full 625 days −$32 | ✅ historical stress, but stronger slippage weakens/turns edge negative | 74 trades / 625 days; forward 0 | 15-pt target floor loses in every window; all in-sample profit is the 12 floor-unaffected trades (`docs/daily22-mes122-retirement-audit-2026-09-23.md`) | Paper evidence only; no broker route; retired as a promotion candidate 2026-09-23 | **BROKEN — execution/fill realism** (downgraded from PROMISING BUT UNPROVEN 2026-09-23) |
...
### 12HR Miyagi
**Evidence verdict:** **WAIT**  
**Execution posture:** research/shadow only; a research detector, not executable or promotable under current account and runtime constraints; parked; not wired into `signal_engine.py`. Nothing is activated.
...
- **Gate attribution (2026-09-07, `docs/miyagi-mnq-gate-attribution-2026-09-07.md`):** per-candidate table for the 8 MNQ causal-stop triggers (2024-08-22 → 2026-02-11): 8/8 fail `rr_below_minimum` (R:R 0.28–0.83), 8/8 fail `stop_too_wide` (140–805 ticks), and **6/8 also fail `MARKET_CONDITION_NOT_TRENDING`** — so even a family stop budget would admit at most the two TRENDING candidates. All eight grade confluence A/A+; confluence is not the failure point. `strat_12hr_miyagi` exists only as a research detector (`research/detector_12hr_miyagi.py`) — it is not wired into `strategy/signal_engine.py` and is not an enabled concept, so no full-engine journal result can exist without inventing strategy logic. Realistic IOC ceiling 5/8 fills, +$266.60 / PF 2.36 at 1 tick, but H1 −$58.44 / H2 +$325.04 — fails chronological-half stability like the bracket (H1 −$56.42 / H2 +$582.33).
- **2026-09-19 causal replay follow-up:** the post-#776 frozen corrected population still had a separate trigger-bar identity defect: `replay_12hr_miyagi_honest_fill.py` excluded the entry bar from stop/T1 resolution even though the written rule makes the fixed stop active immediately. A preregistered same-population audit changes MNQ 2024-09-18 from TARGET to pessimistic same-bar STOP. At 2 ticks the corrected MNQ result is 8 fills, 6W/2L, +$425.33, PF 2.322, H1 +$291.82, H2 +$133.51 (H2 only one fill). MES final P&L is unchanged. See `docs/futures-causal-mechanics-audit-2026-09-19.md`.
- **2026-09-24 evidence note (Operator ruling AFS-0063):** inventory label **WAIT**. PROMISING BUT UNPROVEN was the prior repo label; the AFS ledger records no Operator ruling that set it. #1023's RETIRE rests on an unreproduced handoff-log row. Three of its figures reproduce from the committed 2026-09-19 JSON: 8 MNQ / 10 MES fills; 0/15 MNQ stops within 120 ticks; largest win / net 0.537 MNQ and 0.711 MES. Its p=0.212 and "0/34 reached 2R" need 5m bars that are not in git, and a null method and seed that are not committed in this repo. The #776 lookahead fix holds. RETIRE ("negative/superseded evidence with no approved recovery path") fails because every committed corrected version is net positive at 1-4 ticks. PROMISING ("positive evidence exists, but validation is incomplete") is not supported: MNQ 8 fills, t=1.02; MES 10 fills, t=0.61. Win rates are not significantly above the per-trade geometric breakeven including costs (MNQ 65.8%, MES 73.2%): MNQ 75% (6/8), MES 70% (7/10). The MES win rate is not above 73.2%, and the gap is not significant. There are 18 fills on 14 distinct dates. WAIT ("insufficient evidence or an unresolved prerequisite blocks a stronger evidence classification") is met: insufficient evidence plus unresolved prerequisites. OPEN ITEMS: (a) Stop-anchor replay defect, VERIFIED: the 09-19 replay (`research/replay_12hr_miyagi_honest_fill.py` with `research/detector_12hr_miyagi.py`) uses the 09:30 stop reference (08:00-09:00 bar) as the executable stop for every entry. `12HR_Miyagi_Rules.md` section 6 governs, requiring the most recently completed 60-minute candle before entry, and 5/8 MNQ and 8/10 MES fills enter at or after 10:00 ET. (b) The section 13 weekly Signa gate is unimplemented in the detector and replay. The label will be revisited after a rule-faithful replay-fix re-run. That re-run uses the entry-time stop and either the Signa gate or a disclosure of its absence. Miyagi stays a research detector. Nothing is activated.
...
- **Edge decomposition (2026-09-07, #483):** `RISK_GATES_REMOVE_EDGE`. Signal positive at 3/4 horizons (t 2.06 at 120 min); documented bracket +$2,532.66 / PF 13.6, both halves positive; completed-5m IOC32 gives 20 fills, +$1,859.40 at 1 tick and +$1,838.40 at 3 ticks. Then 34/34 exceed the 120-tick cap (median stop 486 ticks ≈ $243/contract, 16% of the account) and 34/34 fall under 2.0 R:R (median 0.27). The isolated engine rejects 26 at `TREND_STRENGTH_BELOW_REQUIRED` and 5 at `ENTRY_DETACHED_FROM_PRICE` before the cap is ever reached, so the cap is the second wall, not the first. n=34 remains too thin for validation.
- **Trigger-timing A/B (2026-09-18, `docs/322-trigger-timing-ab-2026-09-18.md`):** the rule-defined First Live break is materially earlier than the completed-5m order clock. At the crossing-bar close, 13/34 candidates were >32 ticks adversely detached (median 9, p90 176, max 444). A causal pre-armed stop-entry on the same frozen 34 candidates produces 33 fills / 33 resolved wins / one bracket-invalid no-fill and **+$2,709.66 at 3 adverse ticks**, with H1 +$1,366.34 and H2 +$1,343.32. Twelve resolve on the trigger bar; all twelve have open → trigger → target causally ordered and zero touch both stop and target. Timing classification: **TIMING EDGE SURVIVES / PROMISING BUT UNPROVEN**. This supersedes completed-5m IOC as the final timing model but does not change the account-risk verdict.
...
- **Gate attribution (2026-09-07, `docs/322-mnq-gate-attribution-2026-09-07.md`, `scripts/322_mnq_gate_attribution_2026-09-07.json`):** same 34-candidate population and engine assumptions as the audit, with the bracket P&L split by first engine blocker: `TREND_STRENGTH_BELOW_REQUIRED` 26 candidates (25W / 1L, +$1,998.52), `ENTRY_DETACHED_FROM_PRICE` 5 (+$125.08), `MARKET_CONDITION_NOT_TRADABLE` 1 (+$302.02), `WEAK_BAR_CLOSE` 1 (+$99.02), `RR_BELOW_MINIMUM` 1 (+$8.02) — the groups sum to the +$2,532.66 documented bracket, so the edge is not a residual of any one gate. Structurally 34/34 fail both R:R and the cap (stop p90 878 ticks, max 1,471); both floors-off and frozen engine runs approve 0 and fill 0. Time-exit controls positive at 30/60/120 min (mean MFE 72–126 pts against MAE 44–72 pts), EOD flat. The doc's verdict — BROKEN FOR CURRENT SYSTEM RISK CONSTRAINTS / HOLD, no cap raise, no trend-gate weakening, no separate execution path — is consistent with Option B+.
...
**Verdict: BROKEN — negative evidence** (downgraded 2026-09-08; supersedes PROMISING BUT UNPROVEN)

- **Decision-time replay (2026-09-08, `docs/inverse-orb-decision-time-replay-2026-09-08.md`,
  `scripts/inverse_orb_decision_time_replay.py`):** the same 63 arms, same
  frozen contract, but with the arrival price taken at the decision close
  (`bar_ts + 15m`, look-ahead asserted) and every order routed through the
  production `mirror_order()` → `PaperBroker` path with the #508 bracket
  guard: **41 of 63 fills are `ENTRY_BRACKET_INVALID_AT_FILL`; the 22
  admissible fills net +$29.44, PF 1.14, H1 +$75.18 / H2 −$45.74.** No edge.
- **Why the 2026-09-07 canonical proof is retired:** (1) its fill reference
  was the open of the 5m bar at `bar_ts + 20m`, one full bar after the
  decision candle completes — the same arrival-bar look-ahead the VWAP Hold
  reconciliation rejected; (2) 37 of its 57 fills were held beyond their own
  mirrored stop, carrying +$1,087.74 (more than the whole +$1,026.64), each a
  profitable `STOP_HIT` labelled `LOSS`. #505/#506's post-fill decomposition
  (38 rejected / +$1,138.26; 19 admitted / −$111.62) found the second defect
  first; the decision-time replay is the corrected result and the only
  comparison target from here on.
- **Post-fill decomposition (2026-09-08, #505, `docs/inverse-orb-baseline-post-fill-decomposition-2026-09-08.md`):**
  found the geometry defect first, on the retired `+20m` reference — the
  baseline's entire profit sits in the 38 filled arms the production
  `validate_post_fill` **rejects** (+$1,138.26, 38-0 by P&L, 37 exiting
  `STOP_HIT` recorded as `LOSS`); the 19 admissible arms are −$111.62,
  PF 0.53, H1 −$119.82 / H2 +$8.20. The decision-time replay above is the
  corrected result that supersedes it as the comparison target.
- **The n=111 study (#364) is retired on the same grounds.** Its raw rows are
  not on `main` but survive at commit `42a8618` on tag
  `archive/codex-orb-breakout-marketable-limit-inverse-frozen-2026-07-27`:
  66 of 111 fills are bracket-invalid (+$837.32); the 45 valid fills net
  −$91.60, PF 0.83. Its own attribution table put +$819.60 of +$840.10 on
  "fill selection", not direction.
- Tolerance (#502) and bracket recompute (#509) are both settled and moot:
  the defect is the lane's geometry — a plan level plus a mirrored 50-tick
  stop on a breakout confirmation bar whose close is a median 68 ticks past
  that level. The only legitimate future experiment is a bounded-entry
  variant tested from scratch (a new hypothesis, not a repair); not authorised.
- Forward paper arms since the 2026-09-05 epoch: **0**. Any that arrive must
  be scored under the decision-time contract and reported admissible vs
  rejected separately, never against the retired baselines.
- **Status (2026-09-24 read-only audit; operator confirmed the retirement
  stands): RETIRED / BROKEN. Deployment status: NOT ACTIVE. Evidence status:
  ZERO CURRENT VALID FORWARD EVIDENCE.** Nothing can arrive: #517 put MNQ
  `orb_breakout` on `disabled_concepts_per_instrument` and set it
  `SHADOW_ONLY`, the inverse lane only runs after an MNQ `orb_breakout`
  `TRADE`, and `SCHEDULE_MODE=always_on_shadow` refuses every order. The
  `MNQ_ORB_BREAKOUT_INVERSE_*` env pins still on the box are **ACCOUNTING
  ISOLATION ONLY / NOT ACTIVATION** (the epoch pin scopes main-runner risk
  accounting; do not remove it). **DO NOT REACTIVATE WITHOUT NEW PREREG,
  safety fixes, and replay/live parity proof.** See
  `docs/futures-current-status-2026-09-22.md`.
- The cells below are **provenance only**.
...
- **Canonical population (re-baselined 2026-09-07):** the 63 approved
  ...
  **+$1,026.64, PF 5.28**, H1 +$546.08 / H2 +$480.56, Asian +$63.62 /
  London +$179.90 / New York +$783.12, max DD $55.90. **RETIRED 2026-09-08** — see the verdict bullets above.
...
- **Edge decomposition (2026-09-07, #483):** on the *ungated* 680
  ...
  exactly why n was read as the binding constraint. **Superseded 2026-09-08:**
  n is not the binding constraint; fill admissibility is. The gated positive
  is itself the detached-fill artifact, so the gates were selecting for
  detachment, not for direction. Do not read the ungated negative as a
  refutation, or the gated positive as validation.
...
| ~~4HR Re-Trigger completed-5m IOC replay~~ — historical #483 stage reproduced, but **superseded as the final trigger-timing model on 2026-09-18**. Controlled same-81-candidate A/B: old trigger backfill +$2,886.60; completed-5m IOC8 +$1,266.24 / 37 fills; causal pre-armed touch +$1,414.60 at 1 tick and +$1,294.60 at 3 ticks with both halves positive. Next evidence is prospective natural 1m armed-trigger collection, not another retrospective 5m-close baseline. | prospective trigger-time parity | paper/shadow collection |
...
| ~~**Hypothetical-ledger forward paper lane (B+) build**~~ — **BUILT / DEPLOYED / ACTIVE EVIDENCE COLLECTION.** The isolated wide-stop paper ledgers are live under `WIDE_STOP_LEDGER_MODE=paper_sim` with the preserved epoch `2026-09-09T04:21:04Z`, 1-contract lane rules, IOC-real fills, own risk accounting, and **no promotion path**. `wide_stop_4k` remains the 4HR ledger; the historical path name `wide_stop_6k` is retained for 3-2-2/Miyagi continuity even though the current simulated starting balance is $5,000 per the current handoff. The separately guarded `tradovate_demo` route is armed for eligible wide-stop evidence with matching proof pins and remains isolated from paper evidence. Do not reset the shared epoch merely for cleanup; Daily 2-2 state depends on it. | — | active paper/DEMO evidence collection |
...
| ~~Inverse ORB population reproduction~~ — re-baselined 2026-09-07 on the 63 journal arms; **closed 2026-09-08 by the decision-time replay** (`docs/inverse-orb-decision-time-replay-2026-09-08.md`): the +$1,026.64 / PF 5.28 figure used a fill one 5m bar late and held 37 fills beyond their own stop; corrected result 22 admissible fills, +$29.44, PF 1.14, H2 negative. Sample was never the binding constraint. Verdict → BROKEN | — | — |
| **Forward review checkpoint (set 2026-09-07)** — inverse ORB forward-paper lane and the transition failed-breakdown shadow detector are in *collect* mode: no re-study, re-baseline or new backtest before the checkpoint. Review at the **earlier of 2026-12-07 or 100 IOC-filled inverse ORB arms** (the 100+ preferred sample; canonical set is 63). Report then: inverse ORB fills/armed since 2026-09-07, net, PF, H1/H2 by fill date, session split, top-3-month share, reported as admissible vs `ENTRY_BRACKET_INVALID_AT_FILL` separately under the decision-time contract (`docs/inverse-orb-decision-time-replay-2026-09-08.md`); shadow-detector firings by regime and instrument. The wide-stop lane has its own checkpoint in its spec. **Amended 2026-09-08:** the 63-arm +$1,026.64 / PF 5.28 baseline is RETIRED and must not be used as a comparison target; the lane's standing is BROKEN, so the checkpoint is a report on whether anything admissible was collected, not a promotion review. **Amended 2026-09-24:** the inverse ORB lane is **not in collect mode** — it has been NOT ACTIVE since #517 (2026-09-08) and has zero current valid forward evidence; the inverse half of this checkpoint has nothing to report and is closed. The transition shadow-detector half is unaffected | Inverse ORB promotion (sample); transition reclaim shadow n | Claude Code (report only), operator (decide) |
| ~~3-2-2 First Live trigger-timing parity audit~~ — **done 2026-09-18** (`docs/322-trigger-timing-ab-2026-09-18.md`): exact 34-candidate parity; completed close >32 ticks adversely detached on 13/34; pre-armed First Live +$2,709.66 at 3 ticks with both halves positive. Timing edge survives; sample/risk blockers remain. | — | — |
| 3-2-2 prospective First Live evidence — timing parity is resolved offline; observation-only 1m observer is deployed/active under a proof-critical pin. Collect natural arms/touches under `docs/prereg-forward-one-min-trigger-evidence-review-2026-09-18.md`; no paper-fill authority before the preregistered mechanism threshold. | prospective timing/sample | active observation-only collection |
| ~~**MNQ sustained trend continuation v1 retrospective screen**~~ — **CLOSED 2026-09-22 / WAIT**. Frozen head `cd50f423c77a87255f3b1b19160bb9566f902011`; causal coverage 23,533/23,533 bars. 36 fills, 34 terminal, +$576.18 net, PF 1.762, H1 +$410.54, H2 +$165.64, max DD $172.42. It failed two preregistered gates: >=40 terminal fills and PF >=1.94. No tuning, rerun, session rescue, stop/target change, runner, SHORT mirror, paper/DEMO activation, or runtime integration is authorized. PR #912 preserves the unmerged research/closure record. | retrospective sample / PF | closed; new materially different idea requires new preregistration |
| ~~**Daily 2-2 trigger-timing / entry-identity audit**~~ — **DONE 2026-09-18** (`docs/daily22-trigger-timing-ab-2026-09-18.md`). Activation baseline reproduced exactly: 34 fills, +$13,885.18, PF 2.0171, max DD 25.1528%. Current CME-day completed-close variant: 34 fills, +$13,571.68, PF 1.9482, both halves positive. All 34 fills required a trigger-bar close 2–202 ticks favorable to the planned breakout entry (median 33). Preregistered true-touch variant: **0 admissible fills at 1/2/3 adverse entry ticks** because fixed planned 2R + actual-fill R:R >=2 is mechanically incompatible with adverse touch slippage. Keep completed-close and first-touch as separate strategy identities; any target/R:R/fill redesign requires a new preregistration. | rule identity / entry architecture | operator decision; no runtime change |
| ~~**Miyagi completed-hour timing audit**~~ — **DONE by #776**, but **follow-up causal audit 2026-09-19 found a second replay defect** (`docs/futures-causal-mechanics-audit-2026-09-19.md`): the trigger-touch 5m bar is excluded from immediate stop/T1 resolution. Frozen MNQ 2024-09-18 flips WIN→LOSS; 2-tick MNQ becomes +$425.33 / PF 2.322. Research replay correction/regeneration is **complete repo-side**; runtime remains parked. See `docs/miyagi-trigger-bar-replay-fix-2026-09-19.md`. | — | fixed; no deploy |
| ~~4HR completed-1H stop timing audit~~ — **done 2026-09-18** as part of the pre-armed-touch study. Two hour-boundary MNQ candidates changed stop reference when the stop was restricted to the last 1H candle genuinely complete before the trigger bar, proving the old later-close timestamp could leak a newly-completed 1H bar. | — | — |
...
2. ~~12HR Miyagi detector~~ — detector done (2026-07-26, see
   `12HR_MIYAGI_CANONICAL_EVIDENCE_2026-07-26.md`). Evidence verdict is
   **WAIT** (Operator ruling AFS-0063). It stays a research detector, not
   executable or promotable under current account and runtime constraints.
   Nothing is activated.
3. ~~60M 3-2-2 detector~~ — done, PR #340 (2026-07-26)
4. **Reconcile each detector against manual samples** — before any backtest
5. ~~Initial IOC replay for all three~~ — done 2026-09-07 (#483). **Timing follow-up:** 4HR and 3-2-2 documented touch/First-Live rules were not faithfully represented by waiting for completed 5m closes and now have causal pre-armed offline A/Bs. Miyagi's completed-hour lookahead was fixed by #776, and the 2026-09-19 causal/mechanics audit then found a separate trigger-bar replay defect: immediate stop/T1 is suppressed on the entry bar. That research replay was corrected and regenerated for the trigger-bar defect; the 2026-09-19 JSON remains the last replay committed in this repo. The evidence verdict is **WAIT** (Operator ruling AFS-0063). Miyagi stays a research detector, not executable or promotable under current account and runtime constraints. Current account stop/R:R blockers remain unchanged. Nothing is activated.
