...
| 60M 3-2-2 First Live | ✅ — first live break, no close required | ✅ | ⚠️ full-engine/risk closure #367; **trigger-time parity corrected offline 2026-09-18**, prospective lower-latency proof pending | ✅ pre-armed First Live: 33/34 fills; 3-tick **+$2,471.64**, 32 resolved wins / 0 losses / 1 EOD_BAR_MISSING; completed-5m IOC32 only 20/34 fills / +$1,602.38 at 3 ticks (derived from exit timestamps, pending corpus re-run) | ✅ pre-armed H1 +$1,128.32 / H2 +$1,343.32 at 3 ticks | ✅ pre-armed 3-tick result positive; completed-close IOC also positive | n=34 — thin / consumed; 32/32 corrected resolved wins (1 unresolved) is a small-sample warning, not validation | `RISK_GATES_REMOVE_EDGE` still applies to the real account — 34/34 over cap and 34/34 below 2.0 R:R; **completed-5m timing defect also proven** | Hypothetical paper + guarded DEMO evidence; parked for real-account execution under current risk constraints | **PROMISING BUT UNPROVEN** |
...
- **Edge decomposition (2026-09-07, #483):** `RISK_GATES_REMOVE_EDGE`. Signal positive at 3/4 horizons (t 2.06 at 120 min); documented bracket +$2,532.66 / PF 13.6, both halves positive; completed-5m IOC32 gives 20 fills, +$1,859.40 at 1 tick and +$1,838.40 at 3 ticks. Then 34/34 exceed the 120-tick cap (median stop 486 ticks ≈ $243/contract, 16% of the account) and 34/34 fall under 2.0 R:R (median 0.27). The isolated engine rejects 26 at `TREND_STRENGTH_BELOW_REQUIRED` and 5 at `ENTRY_DETACHED_FROM_PRICE` before the cap is ever reached, so the cap is the second wall, not the first. n=34 remains too thin for validation. These 2026-09-07 booked plan (+$2,532.66) and IOC (+$1,859.40 at 1 tick, +$1,838.40 at 3 ticks) totals include the 2025-01-20 evening-bar target. The corrected trigger-timing record is the next bullet (derived from exit timestamps, pending corpus re-run).
- **Trigger-timing A/B (2026-09-18, `docs/322-trigger-timing-ab-2026-09-18.md`):** the rule-defined First Live break is materially earlier than the completed-5m order clock. At the crossing-bar close, 13/34 candidates were >32 ticks adversely detached (median 9, p90 176, max 444). A causal pre-armed stop-entry on the same frozen 34 candidates produces 33 fills / 32 resolved wins / 1 EOD_BAR_MISSING (2025-01-20 holiday early close) / one bracket-invalid no-fill and **+$2,471.64 at 3 adverse ticks**, with H1 +$1,128.32 and H2 +$1,343.32 (derived from exit timestamps, pending corpus re-run). Twelve resolve on the trigger bar; all twelve have open → trigger → target causally ordered and zero touch both stop and target. Timing classification: **TIMING EDGE SURVIVES / PROMISING BUT UNPROVEN**. This supersedes completed-5m IOC as the final timing model but does not change the account-risk verdict.
...
| ~~3-2-2 First Live trigger-timing parity audit~~ — **done 2026-09-18** (`docs/322-trigger-timing-ab-2026-09-18.md`): exact 34-candidate parity; completed close >32 ticks adversely detached on 13/34; pre-armed First Live +$2,471.64 at 3 ticks (32W-0L, 1 EOD_BAR_MISSING) with both halves positive (derived from exit timestamps, pending corpus re-run). Timing edge survives; sample/risk blockers remain. | — | — |
