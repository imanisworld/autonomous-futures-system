...
| **Hypothetical-ledger forward paper lane (B+)** — isolated paper account explicitly labeled hypothetical $4,000 (4HR) / $6,000 (3-2-2, Miyagi shadow member), 1 contract, family caps 400 / 600 ticks, 4HR R:R ≥ 1.0, IOC-real fills, own daily floor, **no promotion path**; every result reported as "hypothetical ledger", never blended with the $1,500 book. **Lane config spec drafted 2026-09-07:** `docs/wide-stop-hypothetical-ledger-lane-spec-2026-09-07.md` — two ledgers (`wide_stop_4k` / `wide_stop_6k`), decision-bar-close IOC reference, pre-registered checkpoint (earlier of 2026-12-07 or 40 IOC-filled 4HR trades), **D1–D7 approved by the operator 2026-09-07 as recommended** (two ledgers; 10% daily floor; 4HR R:R ≥ 1.0; 8-tick IOC; Miyagi shadow-only; no extra session filter; epoch = first demo release after merge). Build authorized, not started | Build (spec §8), starting with the offline capped-cell IOC expectation | Claude Code (build) |
