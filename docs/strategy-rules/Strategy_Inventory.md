...
> 2. **Armed-trigger day strategies** (4HR MNQ, 3-2-2, Miyagi MNQ): the 2026-09-07 standardized waterfall classified them as `RISK_GATES_REMOVE_EDGE`, and the real-account stop/R:R constraints still bind. **4HR received a material timing correction on 2026-09-18:** the documented retrigger is a level touch, but the prior implementation recognized the touch only from a completed 5m bar and historical replay could fill back at the old trigger. On the same 81 MNQ candidates, old trigger-price backfill was +$2,886.60 / 80 fills; completed-5m IOC8 was +$1,266.24 / 37 fills; a causal pre-armed stop-touch model was +$1,414.60 at 1 tick and +$1,294.60 at 3 ticks, with both halves positive at 3 ticks. Therefore the old claim that 4HR had no fill/parity problem is superseded. 4HR still shows positive evidence after the timing correction, but remains incompatible with current real-account risk and is **PROMISING BUT UNPROVEN / PAPER ONLY**. 3-2-2 and Miyagi still require their own trigger-timing parity audits before inheriting that conclusion.
...
| 60M 3-2-2 First Live | ✅ | ✅ | ✅ full-engine closure (#367) | ✅ 20/34 IOC fills, PF 12.2 (audit, = #340); ❌ 0/34 through production gates | ✅ bracket H1 +$1,383.34 / H2 +$1,149.32; IOC H1 +$1,068.68 / H2 +$790.72 | ✅ 3-tick PF 11.9 | n=34 — thin; winner concentration already flagged | `RISK_GATES_REMOVE_EDGE` — 34/34 over the cap (median stop 486 ticks), 34/34 under 2.0 R:R; engine rejects at `TREND_STRENGTH_BELOW_REQUIRED` (26) first — those 26 carry +$1,998.52 of the +$2,532.66 bracket (attribution doc) | **BROKEN FOR CURRENT SYSTEM RISK CONSTRAINTS — signal and bracket real; PARKED below $6,000 equity (B+, 2026-09-07)** |
...
| **3-2-2 First Live trigger-timing parity audit** — documented rule is a first live break / no candle-close requirement, but current implementation can still infer the touch from completed 5m OHLC. Run the same style of frozen-population completed-5m vs causal pre-armed-touch A/B used for 4HR before treating old IOC evidence as final. | executable timing parity | offline audit first |
| 3-2-2 sample-size expansion — after timing parity is resolved; more 5m sample alone does not answer whether the trigger is late | Strategy verdict | passive collection / later audit |
