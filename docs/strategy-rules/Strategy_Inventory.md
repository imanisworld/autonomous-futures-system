...
> independently reproduced field-for-field with no discrepancy. Result:
> `ioc_close` fills 146/348 (42.0%, vs `ioc_open`'s 105/348, 30.2%) and **all
> three exit modes (static, runner, partial_2ct_approx) pass both-halves-positive
> at every 1/2/3-tick cost tier** — static flips from failing every tier
> under `ioc_open` to passing every tier under `ioc_close`; runner improves
> from $2.69→$4.16/armed (2-tick). Full 9-cell matrix, top-1/3/5 winner
> concentration (41-59% of net P&L across exit modes — a real but
> sample-size-relative concern, not disqualifying), and drawdown-vs-expectancy
> figures in the re-scoring doc. This clears the walk-forward, honest-fill,
> slippage, and sample-adequacy bars that previously blocked this strategy at
> WAIT — verdict upgraded to **PROMISING BUT UNPROVEN** accordingly. Does
> **not** clear VALIDATED: no live/paper evidence exists yet (still
> `SHADOW_ONLY` except the narrow MNQ proof-lane), and — separately, NOT
> resolved by the IOC re-scoring — **which exit mode (static/runner/partial)
> is canonical for `vwap_hold` remains an open question**; `market`+`runner`
> ($10.30/armed, 2-tick, unaffected by this re-scoring) remains the single
> strongest cell system-wide.
...
- Exit mode (static vs runner vs partial): **still unresolved**, a separate
  ...
  by the 2026-07-26 re-scoring
- Next: pick the canonical exit mode for `vwap_hold` (operator decision);
  no further replay/backtest work is required to make that call — the full
