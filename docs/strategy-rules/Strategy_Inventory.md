...
| ORB Breakout — inverted (MNQ evidence lane) | ✅ | ✅ | ✅ | ⚠️ IOC (8-tick marketable tolerance) fills, but 38 of 57 are **inadmissible** — `validate_post_fill` rejects them (`stop_direction` 37, `actual_rr_minimum` 38) | ❌ admissible subset H1 **−$119.82** / H2 +$8.20 (2026-09-08); the +$546/+$480 halves were carried by the rejected arms | ✅ through +4 ticks (#364, n=111 study) | ❌ **19 admissible fills** — below the 30-per-cell minimum; the 63-arm / 57-fill population is 89.5% detached beyond tolerance at fill | Baseline's whole profit is in the rejected set (+$1,138.26, 38-0, 37 `STOP_HIT` labelled `LOSS`); admissible remainder −$111.62 / PF 0.53, negative in every session — static brackets on detached fills, see `inverse-orb-baseline-post-fill-decomposition-2026-09-08.md` | **BROKEN — negative evidence** |
...
- **Post-fill decomposition (2026-09-08, `docs/inverse-orb-baseline-post-fill-decomposition-2026-09-08.md`):**
  the canonical baseline's entire profit sits in the 38 filled arms whose
  fill the production `validate_post_fill` **rejects** (+$1,138.26, 38-0 by
  P&L, 37 of them exiting `STOP_HIT` and recorded as `"result": "LOSS"`
  while carrying positive net). The 19 admissible arms are **−$111.62,
  PF 0.53, H1 −$119.82 / H2 +$8.20, and negative in all three sessions**.
  Mechanism: the 8-tick marketable tolerance bounds only the adverse side,
  so a stale ORB level fills arbitrarily far on the favourable side
  (rejected-set median 91.5 ticks off the nominal entry, max 294) while the
  static stop and target are still computed from the nominal entry — the
  stop ends up between the fill and the target and books a gain. This is the
  2026-09-05 `ENTRY_DETACHED_FROM_PRICE` stale-bracket pathology appearing
  as profit. The cells below are retained as provenance, not as evidence.
...
- **Edge decomposition (2026-09-07, #483):** on the *ungated* 680
  ...
  n is not the binding constraint; admissibility is. The gated positive is
  itself the detached-fill artifact (see the decomposition above), so the
  gates were selecting for detachment, not for direction. Do not read the
  ungated negative as a refutation, or the gated positive as validation.
...
| ~~Inverse ORB population reproduction~~ — **re-baselined 2026-09-07** on the 63 reproducible journal arms (`docs/inverse-orb-canonical-ioc-proof-2026-09-07.md`, +$1,026.64 / PF 5.28, both halves and all sessions positive); the n=111 study is provenance only. Remaining path past PROMISING BUT UNPROVEN is sample: the active forward-paper lane accumulates it — **moot 2026-09-08**: sample is not the binding constraint. The post-fill decomposition (`docs/inverse-orb-baseline-post-fill-decomposition-2026-09-08.md`) shows the baseline's entire profit sits in 38 arms the production validator rejects; the 19 admissible arms are −$111.62 / PF 0.53 with H1 negative. Verdict → BROKEN | — | — |
| **Forward review checkpoint (set 2026-09-07)** — inverse ORB forward-paper lane and the transition failed-breakdown shadow detector are in *collect* mode: no re-study, re-baseline or new backtest before the checkpoint. Review at the **earlier of 2026-12-07 or 100 IOC-filled inverse ORB arms** (the 100+ preferred sample; canonical set is 63). Report then: inverse ORB fills/armed since 2026-09-07, net, PF, H1/H2 by fill date, session split, top-3-month share, all against the 63-arm canonical baseline (+$1,026.64 / PF 5.28); shadow-detector firings by regime and instrument. The wide-stop lane has its own checkpoint in its spec. **Amended 2026-09-08:** the 63-arm baseline is no longer a valid comparison target (post-fill decomposition — its profit is inadmissible). Forward inverse ORB arms must be scored on their own under `validate_post_fill`, reporting admissible vs rejected fills separately; do not blend them with, or compare them to, the +$1,026.64 / PF 5.28 figure | Inverse ORB promotion (sample); transition reclaim shadow n | Claude Code (report only), operator (decide) |
