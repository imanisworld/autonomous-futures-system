...
## Update — 2026-09-21 futures-bot stall fix released (5c91602), gate evidence cron

`futures-bot.service` is now pinned to **`5c91602f3b0c4f2b9a8d9b443660aa156be175a8`** (release dir `5c91602f3b0c-20260920-215643`, live 01:57:02Z, pid 632568). It is a hotfix branch = `c7798d4` + #849 only, promoted under operator GO ("tooling repair"); `origin/main` is deliberately NOT released (it carries unreleased runtime deltas: dashboard evidence UI #831/#832/#836, Webull sandbox adapters #845/#847/#851, `requirements.txt` +1). `options-scanner.service` unchanged on `a0c34818`.

**Root cause (00:15–00:24Z, 00:48–01:19Z, 01:35Z, 01:44Z):** `/status/history?days=N` built each day's row with the full `_dashboard_payload`, which also ran `_diagnostics_payload` → `build_evidence_readiness` (uncached ~13.5k-line evidence parse, ~0.5s) + git subprocesses, then discarded them — inline in an `async def`, so the single event loop was held ~46s at the dashboard's `days=90`. The webhook handler (already `to_thread`) could not be scheduled → TradingView 499 on every post while a dashboard polled → MNQ 15m bars 00:00/00:15/00:45 never evaluated. Not the broker. Fix (#849): journal-only `_daily_summary_payload`, `/status/history` and `/status/today` on `asyncio.to_thread`, regression test. Verified on box: `history?days=14` 8.1s → 0.20s, `/health` 0.002s while history runs, TV posts 200. Integrity 1,394/1,394 pre+post; one TV 502 during the restart window. Rollback: `afs-deploy.sh --release c7798d4993d1ecfd872313cfc5c84da2cda6625d`.

**Operating rule added:** never issue heavy `/status/*` calls (`history?days>14`) against the live process; profile in an isolated python with `LOG_DIR` set. A repro against the live bot at 01:44Z cost the MNQ 01:30 bar.

**Gate question (MNQ +469-pt Sunday-reopen long untraded):** rule, not bug — `require_trending_condition` rejected 2-2 continuation LONGs at 22:30/22:45 (labelled RANGE_BOUND); both would have hit 2R (`docs/counterfactual-gates-off-2026-09-21.md`, #850). On 2026 tape since the 09-16 epoch, MNQ RANGE_BOUND n=58 +0.3R net −$95 vs TRENDING n=62 +7.0R net +$257 → gate stays. `ops/gate_condition_report.py` (#852) now runs daily 22:20Z on the box (cron `afs-gate-condition-report`, shared-dir copy until a release carries the module) and posts the bucket comparison to the Discord daily-report route. Instrument universe remains `['MNQ']`; all other roots are observation-only. No rule changed; decision after 2026-09-30 only if RANGE_BOUND turns and stays net-positive.
