...
Focused verification after the repair: `tests/test_risk_engine.py`, `tests/test_runner_budget_observation.py`, and `tests/test_wide_stop_ledger_paper.py` — **150 passed**; `git diff --check` passed. After reconciliation with current `origin/main` (#824-#827), the full repository suite passed **6,392 tests**. The shared branch-preservation audit was separately optimized to reuse an unchanged source branch path-set across candidate comparisons; preservation semantics remain fail-closed and its focused regression tests pass. This is a safety/maintenance correction only: no strategy parameter, stop/target/R:R rule, instrument universe, broker route, live-trading authority, or wide-stop ledger policy was changed.
