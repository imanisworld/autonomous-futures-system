...
> **CURRENT OPERATOR SUMMARY — supersedes stale deployment/timing statements below:** see `docs/futures-current-status-2026-09-18.md`. Verified active futures release is `ac2b117ec1f98f1470fc54330a4befc913ff15f2`; repository `main` is newer and must not be treated as deployed. Read the current repository HEAD separately when auditing main-vs-release drift rather than freezing a moving `main` SHA into this operator summary. The box remains `LIVE_TRADING_ENABLED=false`, `TRADOVATE_ENV=demo`, `SCHEDULE_MODE=always_on_shadow`, hard cap 1, release-integrity clean, with futures-bot/watcher healthy and the safety timers active. The isolated 1m lane is deployed and proof-pinned. MNQ/MES/M2K/MGC/MCL/MBT all have verified 1m `tf1m` collection. MNQ may use 1m only for already-armed 4HR/3-2-2 observation; the 1m lanes have no setup-discovery or broker authority. No deploy or restart is required merely to catch up to current `main`; `ac2b117ec1f9` is the intentionally minimal #759 evidence release, and #760 is a read-only watcher-only correction.
>
> **Forward response-proof closure — #759/#760:** #759 is now deployed in minimal futures release `ac2b117ec1f98f1470fc54330a4befc913ff15f2` (prior live `6d5b224aa5c208cad0f1d39c09eda13c6171b98b` plus only the append-only observer-response evidence hook). Exact candidate suite: 6,193 passed / 7 skipped; release integrity 1,310/1,310; live remains disabled, Tradovate DEMO remained flat, persisted campaign state survived byte-for-byte, and restart created no synthetic observer row. #760 separately corrected the read-only watcher so collection-only 15m bars cannot manufacture a main-journal stall; only that one hunk was backported to the live watcher (SHA `7d29872d1c85814ec10da50bd3152a57c1eea6873d89c0b1c86fbf8f9ee96563`), futures-bot was not restarted by the watcher fix, and watcher state is now zero BLOCKED with the `ac2b117ec1f9` restart recorded as sanctioned. No further restart is required.
>
...
>
> **Post-session cleanup / research closure — #763/#764/#765/#768:** #763 makes the preserved R5 entry-conditioning audit deterministic and hash-gated. #768 then runs the preregistered one-variable ORB false-break entry A/B and rejects completed signal-bar-close entry as the fix: MNQ +0.0111R/all → -0.0596R/all; MES -0.0416R/all → -0.0847R/all; both halves negative and adverse-tick stress worse. Classification: **NO RUNTIME CHANGE / MIXED_OR_UNSUPPORTED**. The entry-conditioning diagnosis remains real, but any further ORB redesign is a new hypothesis; 2-2 entry-conditioning remains a separate unanswered question. #764 fixes the read-only collector census so closed-session futures silence is `OFF_SESSION` instead of a false DEAD/ABSENT condition; the live watcher already had its own causal session-safe logic, so no deployment/restart is required. #765 records the current collector state. Final cleanup snapshot: local/origin `main=cbac669` after #768, no open PRs, clean merged/closed worktrees materially pruned while dirty/unreviewed worktrees were preserved, and prior dirty local work remains safely anchored at recovery branch `recovery/pre-clean-main-20260918-post754` / `fe4da8d0a9432aeb3c3fe28d57522731e2623a0a`.
>
