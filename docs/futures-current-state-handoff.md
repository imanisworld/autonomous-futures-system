...
_As of 2026-09-20. This is the long futures handoff. Historical audit docs remain evidence records, but they do not override the latest dated current-status file. Repository state is not proof of VPS/deployment state; verify the box separately before claiming anything is running._

> **CURRENT OPERATOR SUMMARY — 2026-09-20:** read `docs/futures-current-status-2026-09-20.md` first. Verified deployed futures release after the latest pass is `98c360da26619407408dbd2bc435540ea37a310e` with release integrity **1,377/1,377**. The box remains `LIVE_TRADING_ENABLED=false`, `TRADOVATE_ENV=demo`, `SCHEDULE_MODE=always_on_shadow`, `MAX_CONTRACTS_HARD_CAP=1`, with broker positions/orders **0/0**, futures-bot/watcher/options scanner active, deploy lock absent, and no watcher BLOCKED findings. The known non-blocking `post_epoch_spans_releases` provenance warning remains.
>
> **4HR continuation treatment — #798:** PR #798 (`780b5ea`) is merged and deployed. Future natural MNQ 4HR 1m observer events now carry read-only `4hr_prearmed_4h22_continuation_v1` metadata using completed ET-wall-clock 4H sequence context. Historical corrected pre-armed scoring shows the treatment subset improved the broad control (+$1,444.58 / PF 2.032 at 1 tick; +$1,402.58 / PF 1.984 at 3 ticks; both halves positive), but top-three-month concentration remains about 94–96%. Classification: **PROMISING BUT UNPROVEN / OBSERVATION ONLY**. It has no paper-fill, DEMO, live, stop/target, risk, or replacement authority.
>
> **Signa Futures Context Lane v1 — #799:** PR #799 (`98c360d`) is merged and deployed. Futures journal context now includes `context.signa_futures_context` for proxy/regime segmentation. It maps futures to proxies (MNQ→QQQ, MES→SPY, M2K→IWM, MYM→DIA, MGC→GLD, MCL→USO+XLE, MBT→BTC) and records tags such as aligned/conflict/mixed/incomplete/missing/error. It is explicitly observation-only: `gate_authoritative=false`, `broker_evaluated=false`, `risk_evaluated=false`, `trade_authorized=false`. Signa is **not** a futures entry trigger, blocking gate, ranking system, or risk/routing authority.
>
> **Current safe next step:** collect natural evidence. On the first future natural 4HR observer event, audit arm timing, true 1m touch, completed-1H stop anchor, 4H continuation metadata, Signa futures context, and zero execution-authority leakage. Do not retune, expand instruments, loosen risk, add Signa gates, or replace the broad 4HR lane because the tags now exist.
>
