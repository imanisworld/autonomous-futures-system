...
> **CURRENT OPERATOR SUMMARY — supersedes stale deployment/timing statements below:** see `docs/futures-current-status-2026-09-18.md`. Verified active futures release is `6d5b224aa5c208cad0f1d39c09eda13c6171b98b`; repository `main=d3ba7a46ddb81558b2181192f718071caa60ce15` is newer and must not be treated as deployed. The box remains `LIVE_TRADING_ENABLED=false`, `TRADOVATE_ENV=demo`, `SCHEDULE_MODE=always_on_shadow`, hard cap 1, release-integrity clean, with futures-bot/watcher healthy and the safety timers active. The isolated 1m lane is deployed and proof-pinned. MNQ/MES/M2K/MGC/MCL/MBT all have verified 1m `tf1m` collection. MNQ may use 1m only for already-armed 4HR/3-2-2 observation; the 1m lanes have no setup-discovery or broker authority. No deploy or restart is required merely to catch up to current `main`; the main-only futures deltas after `6d5b224` are offline replay/backtest qualification and documentation.
>
