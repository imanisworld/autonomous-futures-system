...
The remaining real defect was repaired locally: real-book `max_daily_loss: 150` is now a **fixed account/day realized-loss breaker**, not `$150 × contracts` based on the next setup. Position sizing/per-trade risk remains separate; the 30% global drawdown floor and max-three-trades/day rule are unchanged. Wide-stop isolated ledgers keep their own explicit $300/$600 daily limits and 20% drawdown floors. Focused risk verification: **150 passed**, plus `git diff --check` clean. After reconciliation with the deployed options RTH alert-gate work (#824-#827), the full repository suite passed **6,392 tests**. The branch-preservation audit was also optimized without dropping any preservation check by computing each source branch's changed-path set once and reusing it across candidate-twin comparisons; its focused regression suite passed. No live authority or strategy/execution-route expansion follows from either repair.
