...
**Repository main at this reconciliation:** `e98eeb3f40209430ceb0a15a68020cd194342dab`. Futures runtime fixes #663 and #670 are already merged on `main`; later main-only work must not be inferred as deployed.

**Verified VPS release:** `/root/afs-releases/c538e2bc429d-20260918-074655`, exact commit `c538e2bc429d52c7d960c1d937d5e27ff4361896`, promoted 2026-09-18 11:47Z under the operator-approved one-time freeze waiver. It is a deliberately curated minimal release based on prior deployed `94eb7d3` plus only the #663/#670 runtime/proof delta. Release integrity passed for 1,070 source files; running process cwd, manifest commit, expected commit, release fingerprint and risk-rules hash were independently reconciled after restart.
...
**Wide-stop gap-stop realism (#670) is now deployed in `c538e2bc`.** The forward collector passes the actual 5m bar open into PaperBroker so an already-resting stop that gaps through is priced through the proven `STOP_GAP` path instead of the stale stop level. Long/short and ordinary-stop regressions passed; the exact minimal candidate passed **5,663 tests / 7 skipped**. Pre-deploy `wide_stop_4k` / `wide_stop_6k` paper ledgers were flat and had produced no fills/outcomes in this epoch, so no prior outcome required invalidation. #568 remains closed as superseded.

**Immediate action:** keep the current collection epochs intact and do **not** restart again for #663/#670; both are already live in `c538e2bc`. The September 30 freeze is back in force after this one-time waiver. Existing forward lanes continue under their frozen contracts. Do not retune the active 4HR/3-2-2/Daily/MES/Asia lanes mid-epoch.

**Server drift-gate false alarm fixed (#674 merged as `0c447a0`):** the old VPS cron gate compared repository `main` directly to the curated release and therefore red-alerted on intentional merged-but-unshipped work. The installed `/root/bin/afs-drift-gate.sh` now treats the live release manifest plus durable commit/fingerprint pins as the only red-alarm truth. Current `main` divergence is informational only. A live dry run against `c538e2bc` returned release-integrity clean and **19 merged-but-unshipped items as INFO with exit 0**; a fake wrong commit pin returned exit 1; `--seed` is refused with exit 64. Old server script backup: `/root/bin/afs-drift-gate.sh.pre-curated-20260918T121347Z`. Installed script SHA-256: `94b592d703ead7249382fbe3ab69339849fc1f5e59dbe2bd0da774ebb65167c2`. Cron remains `5 11 * * * /root/bin/afs-drift-gate.sh >/dev/null 2>&1`. No futures-bot restart was required.
