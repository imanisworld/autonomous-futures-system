...
>
> **PR #915 combined-portfolio research — DRAFT / AUDIT ONLY:** #915 is an in-progress research harness for replaying six existing MNQ evidence families through one shared chronological account. It has not produced an accepted combined result and grants no paper, DEMO, live, broker, risk, or deployment authority. Current fixes are confined to the research adapter/tests: 4HR resolved-vs-unresolved provenance, frozen standalone capacity semantics, 3-2-2 frozen corpus identity (`replay_polygon`), and Miyagi frozen 5m evidence corpus identity (`replay_polygon_5m`). These are reproduction/provenance corrections, not reversions of newer corpora or changes to frozen strategy logic. Do not update strategy verdicts or quote combined portfolio economics until all source controls reconcile on one frozen CI-green commit and the fail-closed common-window replay completes.
>
