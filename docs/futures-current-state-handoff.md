...
> **CURRENT OPERATOR SUMMARY — 2026-09-20:** read `docs/futures-current-status-2026-09-20.md` first. Verified deployed futures release after the latest pass is `a02320268e26a05f56b159f03d3cf441e776ef46` with release integrity **1,390/1,390**. The box remains `LIVE_TRADING_ENABLED=false`, `BROKER=tradovate`, `TRADOVATE_ENV=demo`, `SCHEDULE_MODE=always_on_shadow`, `MAX_CONTRACTS_HARD_CAP=1`, with broker position **null**, open P&L **0.0**, no open positions, no working orders, futures-bot/watcher active, failed systemd units **0**, and deploy lock absent. Live preflight is **not armed** (`preflight_failed:heartbeat_fresh`), which does not approve or block this context-only deployment because live trading remains disabled.
>
