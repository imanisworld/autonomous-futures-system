...
The paid Massive options entitlement is working and materially advances the historical backtest path, but it does **not** by itself complete exact replay of the frozen selector. `strat_212_reversal_30m_options` remains **BLOCKED / WAIT**.
...
Massive historical 1-minute aggregates also provide trade volume that can be accumulated causally from market open through the decision time. Cumulative sampled volumes through the decision boundary were 250C=289, 255C=215, 260C=234, 265C=118, 270C=554, 275C=173. These sampled rows clear the selector's minimum-volume threshold of 100.
...
The frozen selector also requires `open_interest` and `delta` on every candidate row.
...
No historical delta/open-interest value is synthesized or back-filled from the current snapshot.

## External evidence paths checked

OCC publishes a Daily Open Interest report by business date. Cboe DataShop states that its option open-interest values use the previous night's OCC end-of-day open interest for the trading day.
...
1. use a causal historical chain snapshot source that contains bid/ask, delta, open interest and interval volume at/before the decision timestamp;
2. run the existing canonical selector unchanged on those frozen bytes;
3. use Massive tick-level historical quotes for exact executable entry/exit fill reconstruction of the selected contract;
4. hash/materialize the quote dataset and manifest through the merged Item-2 tooling;
5. require replay/forward parity and all existing contract/risk gates, including the premium cap.

Until step 1 is solved, the historical data blocker is narrowed but not retired.

## Cost boundary

The existing Massive subscription is already useful: it provides authorized tick-level historical option quotes and historical trade/aggregate data. No additional data purchase should be made merely to continue code work.
