#!/usr/bin/env python3
"""Read-only CLI for Strat direction-authority audit rows."""
...
from ops.direction_authority_audit import build_audit, format_report, report_to_json
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Read-only audit comparing Pine/effective strat_direction with "
            "locally computed Strat direction and selected candidate outcome."
        )
    )
