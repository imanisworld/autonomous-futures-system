...
"""Read-only CLI for the project-check routines.
  ...
  session-start  Repo/worktree/branch/PR/runtime snapshot, including a
                 live (no-fetch) origin/main freshness check and worktree-
  ...
  precommit      Strictly read-only. Compares current repo state against the
                 ...
                 including detached HEAD or a branch registered to more than
                 one worktree.
...
def _cmd_session_start(args: argparse.Namespace) -> int:
    ...
    lom = repo["live_origin_main_verification"]
    print(f"  live origin/main:     {lom['freshness']}")
    wo = repo["worktree_ownership"]
    print(f"  worktree ownership ok: {wo['ok']}")
...
def _cmd_daily(args: argparse.Namespace) -> int:
    ...
    if tc["status"] == "PASS":
        s = tc["summary"]
        ec = tc["execution_context"]
        tolerance_summary = ", ".join(
            f"{root}={info.get('effective_replay_paper')}"
            for root, info in (ec["live_entry_tolerance_ticks"] or {}).items()
        ...
        print(
            ...
            f"  0 fills exceeding live entry tolerance (fill_model={ec['live_entry_fill_model']}, tolerance_ticks: {tolerance_summary})\n"
        )
    else:
