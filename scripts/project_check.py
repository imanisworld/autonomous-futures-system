...
"""Read-only CLI for the project-check routines.
  ...
  session-start  Repo/worktree/branch/PR/runtime snapshot, including a live
                 ownership/origin-freshness preflight (read-only `git
                 ls-remote`, no fetch); writes a small session-state cache
...
def _cmd_session_start(args: argparse.Namespace) -> int:
    ...
    print(f"  ownership preflight:  {'PASS' if op.get('ok') else 'FAIL_CLOSED'}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    p_session = sub.add_parser(
        "session-start",
        help="Repo/worktree/branch/PR/runtime snapshot, incl. ownership/origin-freshness preflight",
    )
