...
Three routines total:
  ...
  session-start  Routine 1 (mode A): repo/worktree/branch/PR/runtime
                 snapshot, plus worktree-ownership and origin/main-freshness
                 checks (reused from ops.project_check.preflight -- never
                 fetches, ls-remote only). Writes a small session-state cache
                 under .git/ so `precommit` can compare against it later.
                 Otherwise read-only.
  precommit      Routine 1 (mode B): strictly read-only. Compares current
                 repo state against the last session-start snapshot and
                 fails closed on drift, including worktree-ownership drift.
  ...
  daily          Routine 3: Daily Reconciliation + Trade Chain Integrity:
                 ...
                 accounting (incl. per-fill entry model + effective
                 tolerance) since the prior checkpoint. Read-only against
...
def _cmd_session_start(args: argparse.Namespace) -> int:
    ...
    print(f"  worktree ownership ok: {ownership['ok']}")
    for err in ownership["errors"]:
        print(f"    - {err}")
    main_live = report["origin_main_live_check"]
    print(f"  origin/main freshness: {main_live['freshness']}")
    if main_live["freshness"] != "CURRENT":
        print(f"    - {main_live['detail']}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    p_session = sub.add_parser(
        "session-start",
        help="Repo/worktree/branch/PR/runtime snapshot, incl. worktree ownership + origin/main freshness",
    )
