...
"""Read-only CLI for the project-check routines.
  ...
  session-start  Routine 1 (Session Safety + Runtime Snapshot), mode A.
                 Repo/worktree/branch/PR/runtime snapshot; also live-verifies
                 origin/main against the remote (one `ls-remote`, no fetch)
                 and checks worktree/branch ownership. Writes a small
                 session-state cache under .git/ so `precommit` can compare
                 against it later. Otherwise read-only.
  precommit      Routine 1 (Session Safety + Runtime Snapshot), mode B.
                 Strictly read-only. Compares current repo state against the
                 last session-start snapshot and fails closed on drift, a
                 detached HEAD, or a branch double-registered to two
                 worktrees.
  promotion      Routine 2: Strategy Promotion Proof Gate. Accounting-identity
                 + safety-gate validator over an explicit evidence-facts file
                 (see ops/project_check/promotion.py's module docstring for
                 the schema). Read-only.
...
def _cmd_session_start(args: argparse.Namespace) -> int:
    ...
    main_verified = repo["origin_main_verified"]
    print(
        f"  origin/main verified: {main_verified['freshness']}"
        f" (HEAD contains it: {main_verified['head_contains_verified_main']})"
    )
    print(f"  upstream:             {repo['upstream']}")
    ownership = repo["worktree_ownership"]
    print(f"  worktree ownership ok: {ownership['ok']}  detached HEAD: {ownership['detached_head']}")
    if ownership["errors"] or ownership["duplicate_branch_owners"]:
        for err in ownership["errors"]:
            print(f"    - {err}")
        for dup in ownership["duplicate_branch_owners"]:
            print(f"    - branch {dup['branch']!r} registered to multiple worktrees: {dup['paths']}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    p_session = sub.add_parser(
        "session-start",
        help="Repo/worktree/branch/PR/runtime snapshot; also verifies origin/main and worktree ownership",
    )
