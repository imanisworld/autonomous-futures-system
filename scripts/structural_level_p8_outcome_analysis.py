...
FROZEN = {
    "MNQ": {"candidates": "148768cd9a7b9b6f48d003d91cdf73b757b3018dca1343dc9a56b7e4f91c1a7a",
            "features": "1bfd7edd3e14ae8c8ea09497b678e69b8f753bbf55ca04fcbd8c9f76c44c0503",
            "outcomes": "b8e86c64cffb12eeb812864b02b1e3f32fbc729f46602efab46e1f9228413c60"},
    "MES": {"candidates": "e50fa5525422690ceff69a4a115c78517eb9cdec51f8dee5ee004733045a7ebe",
            "features": "eda7cfaf1dc24801b6b94a8e999625a87faefaa9b1c4d577cfcf257d7a84e6b1",
            "outcomes": "326ae59f9ff13590716d27eda0c25031615864d348ffbbd1871b95f80250bc08"},
}
...
def main() -> None:
    ...
    # pooled MNQ+MES descriptive (strata printed above it in the record; never a stand-alone statement)
    pooled = {}
    for h in ("H1", "H2", "H3", "H5", "H6"):
        eff = [result["instruments"][i]["hypotheses"][h]["contrast"].get("effect_R") for i in INSTRUMENTS]
        ns = [result["instruments"][i]["hypotheses"][h]["contrast"].get("n", 0) for i in INSTRUMENTS]
        if all(e is not None for e in eff):
            pooled[h] = {"effect_R_n_weighted": fmt(sum(e * n for e, n in zip(eff, ns)) / sum(ns)),
                         "per_instrument": dict(zip(INSTRUMENTS, eff)), "sign_agreement": bool((eff[0] > 0) == (eff[1] > 0))}
        else:
            pooled[h] = {"effect_R_n_weighted": None, "per_instrument": dict(zip(INSTRUMENTS, eff)), "sign_agreement": None}
