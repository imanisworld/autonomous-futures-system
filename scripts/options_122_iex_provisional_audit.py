#!/usr/bin/env python3
"""Audit a miss-allowed IEX provisional 1-2-2 observer against delayed SIP.

Research/evidence only. The population is every structurally ARMED 1-2-2 watch
...
AUDIT_ID = "OPTIONS_122_IEX_PROVISIONAL_RECONCILIATION"
...
def _extract_arms(snapshot_dir: Path, manifest: Mapping[str, Any]) -> tuple[list[Arm], dict[tuple[str, str], Sequence[Any]]]:
        ...
        for symbol in symbols:
            ...
            for index, current in enumerate(series.series):
                ...
                if armed.reference_direction not in {"two_up", "two_down"}:
                    raise RuntimeError("122 arm missing reference direction")
                arms.append(
                    Arm(
    ...
    if len(ids) != len(set(ids)):
        raise RuntimeError("duplicate 122 arm identity")
    if len(arms) != EXPECTED_ARMS:
        raise RuntimeError(f"frozen 122 arm count mismatch: expected={EXPECTED_ARMS} actual={len(arms)}")
...
def _summarize(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "claims_not_made": [
            ...
            "1-2-2 option expectancy",
            "1-2-2 strategy promotion",
            "DEMO eligibility",
...
async def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    result = {
        ...
        "preregistration": "docs/options-122-iex-provisional-preregistration-2026-09-18.md",
        ...
        "policy": {
            ...
            "provisional_emission": "first IEX boundary break must be 1-2-2 reversal",
