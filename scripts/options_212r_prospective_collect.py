...
def _history_sessions(day: date, lookback_days: int = 10) -> list[Any]:
    """Return recent sessions including the prior week for opening-bar precursors."""
    out = []
...
def _load_journal(path: Path) -> tuple[dict[str, datetime], set[str]]:
    armed: dict[str, datetime] = {}
    terminal: set[str] = set()
    if not path.exists():
        return armed, terminal
...
async def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    armed_seen, terminal_seen = _load_journal(journal)
    ...
    history_sessions = _history_sessions(session.date)
