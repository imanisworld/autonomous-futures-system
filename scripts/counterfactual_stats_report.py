...
repeatable and explicit, including fill/no-fill counts, PF, H1/H2, drawdown,
MAE/MFE when supplied, break-even round-trip cost, and labelled hypothetical
cost sensitivity.
...
Input JSONL contract (one candidate per row):
  ...
  mae_r: number|null          optional
  mfe_r: number|null          optional
  ts: str                     optional provenance only

The reporter never assigns a candidate to a cohort; doing so here would invent
research logic that belongs in the study that produced the row.
"""
...
def validate_rows(rows: Iterable[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for index, raw in enumerate(rows, start=1):
        ...
        if row["filled"]:
            ...
            row["pnl_dollars"] = _number(row.get("pnl_dollars"), label=f"row {index} pnl_dollars")
...
def summarize_cohort(rows: list[dict[str, Any]], hypothetical_costs: Iterable[float]) -> dict[str, Any]:
    ...
    costs = [float(value) for value in hypothetical_costs]
    return {
        "candidates": len(rows),
        "fills": fills,
        "no_fills": len(rows) - fills,
        "fill_rate_percent": round((fills / len(rows)) * 100.0, 2) if rows else None,
        "wins": wins,
        "losses": losses,
        ...
        "h1_pnl_dollars": round(sum(pnls[:midpoint]), 2),
        "h2_pnl_dollars": round(sum(pnls[midpoint:]), 2),
...
def build_report(rows: Iterable[dict[str, Any]], *, hypothetical_costs: Iterable[float] = (0.0, 2.0, 4.0)) -> dict[str, Any]:
    ...
    return {
        ...
        "cost_note": "Cost sensitivity is hypothetical; no commission value is inferred or configured by this reporter.",
        "rows": len(clean),
        "cohorts": {
            cohort: summarize_cohort(grouped[cohort], hypothetical_costs)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("inputs", nargs="+", help="Counterfactual JSONL file(s), one candidate per row")
    parser.add_argument(
        "--hypothetical-cost",
        type=float,
        action="append",
    ...
    report = build_report(_load_jsonl([Path(value) for value in args.inputs]), hypothetical_costs=costs)
