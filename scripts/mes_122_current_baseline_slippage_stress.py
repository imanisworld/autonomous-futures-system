...
Evidence only. Reuses the fixed-one-contract controlled harness from PR #521.
The strategy itself is unchanged; only PaperBroker adverse slippage varies
across 1, 2, and 3 ticks. $1.48 round-trip commission remains applied at the
analysis layer by the shared metric helper.
"""
...
def run(out_path: Path) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="mes122_slippage_") as tmp:
        ...
        for slip in SLIPPAGE_TICKS:
            ...
            results["slippage"][name] = _summarize_run(runs[name])
    ...
    results["robust_through_3t"] = all(
        all(results["screen"][name][key] for key in (
            "net_positive", "pf_above_1", "h1_positive", "h2_positive"
        ))
        for name in ("slip_1t", "slip_2t", "slip_3t")
    )
