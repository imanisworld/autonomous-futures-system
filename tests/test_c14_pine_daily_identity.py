...
def _replay_vwap_series(bars: list[dict]) -> list[float]:
...
class TestRequiredBoundaries:
    ...
    def test_replay_boundary_matches_pine(self, inst_windows, window, et, expect_reset):
        ...
        replay_reset = i in detect_day_boundaries(_bars(rows))
    ...
    def test_sep07_1800_is_same_trade_date_as_sep06_1800(self, inst_windows):
        ...
        assert cme_trading_day(sun["ts"]) == cme_trading_day(mon["ts"]) == date(2026, 9, 8)
        assert cme_trading_day(tue["ts"]) == date(2026, 9, 9)
...
class TestDailyIdentityAllWindows:
    def test_trading_day_equals_pine_time_tradingday_on_every_bar(self, inst_windows):
        ...
        for rows in windows.values():
            for r in rows:
                assert cme_trading_day(r["ts"]) == r["pine_tradingday"], r["et"]

    def test_detect_day_boundaries_equals_pine_daily_bar_changes(self, inst_windows):
        _, windows = inst_windows
        for name, rows in windows.items():
            assert detect_day_boundaries(_bars(rows)) == _pine_boundaries(rows), name
    ...
    def test_holiday_reopen_is_the_only_1800_without_reset(self, inst_windows):
        ...
        for name, rows in windows.items():
            boundaries = set(detect_day_boundaries(_bars(rows)))
            for i, r in enumerate(rows):
    ...
    def test_resample_daily_uses_the_same_identity(self, inst_windows):
        _, windows = inst_windows
        for name, rows in windows.items():
            daily = resample_daily(_bars(rows))
            expect = sorted({cme_trading_day(r["ts"]) for r in rows})
            assert [cme_trading_day(d["ts"]) for d in daily] == expect, name
...
class TestVwapAndHodLod:
    ...
    def test_no_hod_lod_reset_on_sep07_1800(self, inst_windows):
        ...
        by_et = {datetime.fromisoformat(c["timestamp"]).astimezone(_ET): c
                 for c in derive_candles(_bars(rows), "MES", 15)}
        c = by_et[rows[i]["et"]]
...
class TestCmeTradingDayHelper:
    ...
    def test_good_friday_is_not_a_non_trade_date(self):
        ...
        assert cme_trading_day(datetime(2026, 4, 2, 18, 0, tzinfo=_ET)) == date(2026, 4, 3)
    ...
    def test_ordinary_weekday_and_sunday_reopen_unchanged(self):
        ...
        assert cme_trading_day(datetime(2026, 6, 9, 17, 59, tzinfo=_ET)) == date(2026, 6, 9)
        assert cme_trading_day(datetime(2026, 6, 9, 18, 0, tzinfo=_ET)) == date(2026, 6, 10)
        # Sun 2026-06-14 18:00 → Mon 06-15; Fri 06-12 16:59 → 06-12.
        assert cme_trading_day(datetime(2026, 6, 14, 18, 0, tzinfo=_ET)) == date(2026, 6, 15)
        assert cme_trading_day(datetime(2026, 6, 12, 16, 59, tzinfo=_ET)) == date(2026, 6, 12)
    ...
    def test_holiday_eve_reopen_maps_to_next_trade_date(self):
        ...
        assert cme_trading_day(datetime(2026, 11, 25, 18, 0, tzinfo=_ET)) == date(2026, 11, 27)
        assert cme_trading_day(datetime(2026, 11, 26, 18, 0, tzinfo=_ET)) == date(2026, 11, 27)
        # Christmas 2026-12-25 (Fri): Thu 12-24 12:00 → 12-24; Sun 12-27 18:00 → Mon 12-28.
        assert cme_trading_day(datetime(2026, 12, 24, 12, 0, tzinfo=_ET)) == date(2026, 12, 24)
        assert cme_trading_day(datetime(2026, 12, 27, 18, 0, tzinfo=_ET)) == date(2026, 12, 28)

    def test_accepts_unix_int_and_datetime(self):
        dt = datetime(2026, 9, 7, 18, 0, tzinfo=_ET)
        assert cme_trading_day(int(dt.timestamp())) == cme_trading_day(dt) == date(2026, 9, 8)
    ...
    def test_dst_boundary_does_not_shift_reset(self):
        ...
        assert cme_trading_day(datetime(2026, 11, 1, 17, 59, tzinfo=_ET)) == date(2026, 11, 2)
        assert cme_trading_day(datetime(2026, 11, 1, 18, 0, tzinfo=_ET)) == date(2026, 11, 2)
        assert cme_trading_day(datetime(2026, 10, 30, 16, 59, tzinfo=_ET)) == date(2026, 10, 30)
