...
retrace requiring a close not a wick; retrace deadline before 9:30 AM ET; price
already through the trigger at 9:30; entry window; target; fixed structural
references; timezone/DST behavior; missing bars and malformed inputs.
...
def _calls_8am_break(day, high=98.0, low=80.0):
    # breaks ABOVE 4AM high (95)
    return _bar(day, 8, 0, 90.0, high, low, 85.0)
...
def _puts_8am_break(day, high=110.0, low=88.0):
    # breaks BELOW 4AM low (92)
    return _bar(day, 8, 0, 100.0, high, low, 95.0)
...
def _basic_calls_bundle(day=_TUE, prior_day=_MON, instrument="MNQ"):
    bars_4h = [_prior_4pm(prior_day), _calls_4am(day), _calls_8am_break(day)]
    bars_5m = _five_min_seq(day, [
        (8, 0, 90, 93, 89, 93),     # still above 4AM high (95)? no -- irrelevant, above trigger not yet retraced
        (8, 5, 93, 96, 92, 96),     # close 96 > 95, no retrace yet
        (8, 10, 96, 97, 93, 93),    # close 93 < 95 -> FIRST retrace confirmation
        (8, 15, 93, 94, 90, 91),
        (9, 25, 91, 92, 89, 90),
...
def _basic_puts_bundle(day=_TUE, prior_day=_MON, instrument="MNQ"):
    bars_4h = [_prior_4pm(prior_day), _puts_4am(day), _puts_8am_break(day)]
    bars_5m = _five_min_seq(day, [
        (8, 0, 95, 96, 92, 91),     # close 91 < 92 (4AM low), no retrace yet
        (8, 5, 91, 93, 90, 90),     # close 90, still below 92
        (8, 10, 90, 94, 89, 93),    # close 93 > 92 -> FIRST retrace confirmation
        (8, 15, 93, 95, 91, 94),
        (9, 25, 94, 96, 93, 95),
...
def test_monday_mnq_uses_sunday_reference():
    ...
    bars_4h = [_prior_4pm(sunday, high=200.0, low=180.0), _calls_4am(monday, high=195.0, low=175.0),
               _calls_8am_break(monday, high=198.0)]
    bars_5m = _five_min_seq(monday, [
        (8, 10, 196, 197, 190, 190),   # close 190 < 195 -> retrace
...
def test_monday_qqq_uses_friday_reference():
    ...
    bars_4h = [
        ...
        _calls_8am_break(monday, high=298.0),
    ]
    bars_5m = _five_min_seq(monday, [
        (8, 10, 296, 297, 290, 290),
...
def test_4am_inside_bar_rejected():
    ...
    bars_4h = [prior, inside, _bar(_TUE, 8, 0, 95, 99, 91, 96)]
...
def test_4am_outside_bar_rejected():
    ...
    bars_4h = [prior, outside, _bar(_TUE, 8, 0, 95, 106, 84, 96)]
    result = detect_4hr_retrigger(bars_4h, [], [], _TUE, "MNQ")
    assert result is None
...
def test_4am_exact_equality_boundaries_are_inside_not_calls_or_puts():
    ...
    bars_4h = [prior, exact, _bar(_TUE, 8, 0, 95, 101, 89, 96)]
    result = detect_4hr_retrigger(bars_4h, [], [], _TUE, "MNQ")
    assert result is None


# ── 6. 8AM reversal requirement ────────────────────────────────────────────────

def test_8am_fails_to_break_4am_high_rejects_calls_setup():
    prior = _prior_4pm(_MON)
    four_am = _calls_4am(_TUE)  # high=95
    eight_am_no_break = _bar(_TUE, 8, 0, 90, 94.0, 80, 85)   # high 94 < 95, no break
    bars_4h = [prior, four_am, eight_am_no_break]
    result = detect_4hr_retrigger(bars_4h, [], [], _TUE, "MNQ")
    assert result is None


def test_8am_fails_to_break_4am_low_rejects_puts_setup():
    prior = _prior_4pm(_MON)
    four_am = _puts_4am(_TUE)  # low=92
    eight_am_no_break = _bar(_TUE, 8, 0, 100, 110, 93.0, 95)  # low 93 > 92, no break
    bars_4h = [prior, four_am, eight_am_no_break]
    result = detect_4hr_retrigger(bars_4h, [], [], _TUE, "MNQ")
    assert result is None
...
def test_retrace_wick_only_does_not_confirm():
    """A 5-min bar whose LOW pokes below the 4AM high intrabar, but whose CLOSE
    stays above it, must NOT confirm the retrace."""
    bars_4h = [_prior_4pm(_MON), _calls_4am(_TUE), _calls_8am_break(_TUE)]
    bars_5m = _five_min_seq(_TUE, [
        (8, 5, 96, 97, 90, 96),   # low=90 wicks below 95, but CLOSE=96 stays above -> no confirm
        (9, 30, 96, 97, 95, 96),
    ])
...
def test_retrace_close_confirms():
    bars_4h = [_prior_4pm(_MON), _calls_4am(_TUE), _calls_8am_break(_TUE)]
    bars_5m = _five_min_seq(_TUE, [
        (8, 5, 96, 97, 94, 94),   # CLOSE 94 < 95 -> confirms
        (9, 30, 94, 95, 93, 94),
    ])
    result = detect_4hr_retrigger(bars_4h, bars_5m, [_bar(_TUE, 8, 0, 90, 94, 88, 91)], _TUE, "MNQ")
    assert result is not None
...
def test_retrace_at_or_after_930_does_not_count():
    ...
    bars_4h = [_prior_4pm(_MON), _calls_4am(_TUE), _calls_8am_break(_TUE)]
    bars_5m = _five_min_seq(_TUE, [
        (8, 5, 96, 97, 94, 96),    # stays above 95, no retrace yet
        (9, 30, 96, 97, 90, 90),   # closes below 95 but AT 9:30 -- too late
    ])
    result = detect_4hr_retrigger(bars_4h, bars_5m, [_bar(_TUE, 8, 0, 90, 94, 88, 91)], _TUE, "MNQ")
    assert result is None
...
def test_dst_spring_forward_boundary():
    ...
    bars_4h = [_prior_4pm(sunday, high=100.0, low=90.0), _calls_4am(monday), _calls_8am_break(monday)]
    bars_5m = _five_min_seq(monday, [(8, 10, 96, 97, 93, 93), (9, 30, 93, 94, 91, 93)])
...
def test_missing_8am_bar_returns_none():
    bars_4h = [_prior_4pm(_MON), _calls_4am(_TUE)]  # no 8AM bar
    assert detect_4hr_retrigger(bars_4h, [], [], _TUE, "MNQ") is None


def test_missing_prior_4pm_bar_returns_none():
    bars_4h = [_calls_4am(_TUE), _calls_8am_break(_TUE)]  # no prior 4PM bar
    assert detect_4hr_retrigger(bars_4h, [], [], _TUE, "MNQ") is None
