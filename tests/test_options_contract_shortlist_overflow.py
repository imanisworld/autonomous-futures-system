...
def test_huge_numeric_candidate_fails_closed_without_overflow():
    result = shortlist_contracts(
        ContractSelectionRequest(
            ticker="AAPL",
            direction="CALL",
            candidates=(_candidate(strike=10**10000),),
            policy=_policy(),
        )
    )
...
def test_huge_numeric_policy_is_invalid_request_without_overflow():
    result = shortlist_contracts(
        ContractSelectionRequest(
            ...
            policy=_policy(max_premium_per_share=10**10000),
        )
    )

    assert result.status == "INVALID_REQUEST"
    assert any("max_premium_per_share" in reason for reason in result.blocking_reasons)
