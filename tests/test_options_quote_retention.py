...
def test_valid_quote_is_ok_and_carries_rule_provenance():
    ...
    assert record.reason_code == "quote_retained"
    assert record.rule_sha256 == _rule_sha()
    assert record.quote_age_seconds == 60.0


def test_missing_quote_timestamp_is_retained_as_missing():
    record = _retain(quote_ts=None)
...
def test_missing_bid_or_ask_is_retained_as_missing():
    bid_missing = _retain(bid=None)
    ask_missing = _retain(ask=None)
    assert bid_missing.status == "MISSING"
    assert ask_missing.status == "MISSING"


def test_stale_quote_is_retained_but_not_ok():
    record = _retain(quote_ts="2026-09-18T13:40:00+00:00")
    assert record.status == "STALE"
    assert record.reason_code == "quote_stale"
    assert record.quote_age_seconds == 1260.0


def test_future_quote_is_retained_but_not_ok():
    record = _retain(quote_ts="2026-09-18T14:02:00+00:00")
    assert record.status == "FUTURE"
    assert record.reason_code == "quote_after_decision"


def test_wide_spread_is_recorded_and_not_ok():
    record = _retain(bid=1.0, ask=2.0)
    assert record.status == "WIDE_SPREAD"
    assert record.reason_code == "spread_too_wide"
    assert record.spread_percent > 20.0


def test_free_text_source_is_rejected():
    record = _retain(source="some vendor endpoint")
    assert record.status == "INVALID"
    assert record.reason_code == "source_not_allowed"


def test_naive_timestamp_is_invalid():
    record = _retain(quote_ts="2026-09-18T14:00:00")
    assert record.status == "INVALID"
    assert record.reason_code == "timestamp_invalid"


def test_invalid_liquidity_numeric_types_fail_closed():
    record = _retain(volume=True)
    assert record.status == "INVALID"
    assert record.reason_code == "liquidity_field_invalid"


def test_canonical_json_is_byte_stable():
    first = quote_record_json(_retain()).encode("utf-8")
    second = quote_record_json(_retain()).encode("utf-8")
    assert first == second
    assert hashlib.sha256(first).hexdigest() == hashlib.sha256(second).hexdigest()


def test_manifest_is_reproducible_and_sorted():
    a = quote_record_json(_retain(contract_id="A")).encode()
    b = quote_record_json(_retain(contract_id="B")).encode()
    files_1 = {"quotes/b.jsonl": b, "quotes/a.jsonl": a}
    files_2 = {"quotes/a.jsonl": a, "quotes/b.jsonl": b}
    first = build_quote_manifest(files_1, rule=_rule(), rule_sha256=_rule_sha())
    second = build_quote_manifest(files_2, rule=_rule(), rule_sha256=_rule_sha())
    assert first == second
...
def test_rule_rejects_unknown_allowed_source():
    ...
    try:
        retention_rule_from_mapping(raw)
    except ValueError as exc:
