...
def test_aggregates_with_producer_defined_halves_and_chronological_drawdown():
    # Deliberately shuffled input. Chronological A P&L is -100, +200, -50,
    # so max drawdown is 100 rather than whatever input order happens to imply.
    rows = [
        _row(ts="2026-09-01T11:00:00+00:00", pnl_dollars=200.0, mae_r=0.5, mfe_r=2.0),
        _row(
        ...
        _row(ts="2026-09-01T10:00:00+00:00", pnl_dollars=-100.0, mae_r=1.0, mfe_r=0.5),
        _row(
    ...
    assert report["chronology_source"] == "input.ts"
    ...
    assert a["max_drawdown_dollars"] == 100.0
