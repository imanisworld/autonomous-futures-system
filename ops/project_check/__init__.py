...
1. Session Safety + Runtime Snapshot   -> ops.project_check.session
   (session-start and precommit modes; both reuse gitutil.worktree_ownership()
   and gitutil.verified_origin_main() for branch/worktree-collision and
   stale-main detection -- this folds in what used to be a separate
   "ownership preflight" routine.)
