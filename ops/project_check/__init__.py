...
1. Session Safety + Runtime Snapshot   -> ops.project_check.session
   (two modes: session-start, precommit -- also live-verifies origin/main
   and worktree/branch ownership, folding in what used to be a separate
...
3. Daily Reconciliation + Trade Chain  -> ops.project_check.daily
   (folds in trade-chain integrity      -> ops.project_check.trade_chain)
