...
1. Session Safety + Runtime Snapshot   -> ops.project_check.session
   (session-start and precommit modes both reuse the worktree-ownership and
   origin/main-freshness checks in ops.project_check.preflight -- ownership
   preflight used to be a separate fourth routine; it is folded into this
   one so the system has exactly three routines total)
