...
1. Session Safety + Runtime Snapshot   -> ops.project_check.session
   (session-start mode also live-verifies origin/main freshness and
   worktree ownership via ops.project_check.gitutil)
2. Strategy Promotion Proof Gate       -> ops.project_check.promotion
   (also runs the same origin/main + worktree-ownership provenance gate
   as a hard safety cap on evidence)
