...
def _remote_main_sha(ls_remote_output: str) -> str | None:
...
def verified_origin_main(root: Path) -> dict[str, Any]:
    """Compare local origin/main to the LIVE remote (via ls-remote) without
    fetching or otherwise updating either side. Unlike main_sync_state()
    (which only compares against the last-fetched remote-tracking ref), this
    catches a locally stale origin/main -- i.e. accidental work against a
    main that has since moved upstream."""
    local = run_git_result(["rev-parse", "--verify", "refs/remotes/origin/main^{commit}"], cwd=root)
    remote = run_git_result(["ls-remote", "--heads", "origin", "refs/heads/main"], cwd=root)
    ...
    if freshness == "CURRENT":
        ancestry = run_git_result(
            ["merge-base", "--is-ancestor", "refs/remotes/origin/main", "HEAD"], cwd=root
        )
...
def worktree_ownership(root: Path) -> dict[str, Any]:
    """Verify the current branch uniquely belongs to the current worktree and
    that no branch is registered to more than one worktree -- the specific
    collision that causes wrong-branch/wrong-worktree commits."""
