...
def verified_origin_main(root: Path) -> dict[str, Any]:
    """Compare local origin/main to the LIVE remote (via `ls-remote`, which
    reads without writing any ref) rather than trusting the local
    remote-tracking ref, which only reflects the last fetch. Unlike
    `main_sync_state`, this can tell a genuinely current origin/main apart
    from one that is merely locally cached and stale."""
...
def worktree_ownership(root: Path) -> dict[str, Any]:
    """Verify the checked-out branch uniquely belongs to the current worktree
    -- catches a branch registered to two worktrees at once and a detached
    HEAD, both of which make "which worktree owns this branch" ambiguous."""
