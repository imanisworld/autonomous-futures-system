...
def verified_origin_main(root: Path) -> dict[str, Any]:
    ...
    Deliberately does one `git ls-remote` (no fetch, no ref update) so a
    caller can tell "local origin/main is fresh" from "local origin/main is
    stale, and I don't know it" -- `main_sync_state` above only ever compares
    against the locally cached remote-tracking ref, which silently goes stale
    between fetches.
    """
...
def worktree_ownership(root: Path) -> dict[str, Any]:
    ...
    Flags a detached HEAD (no auditable branch ownership), the current
    worktree being absent from `git worktree list --porcelain`, the current
    worktree not owning its own checked-out branch, and a branch registered
    to more than one worktree at once.
    """
