"""Read-only ownership/base-freshness checks, folded into Routine 1 (Session
Safety + Runtime Snapshot: see ops.project_check.session) rather than exposed
as a separate top-level routine -- this repo keeps exactly three routines
(session/precommit, promotion, daily+trade-chain).

Neither function here fetches or modifies refs. `verified_origin_main`
compares the locally known ``origin/main`` SHA with the SHA currently
advertised by the remote via a read-only `git ls-remote`. `worktree_ownership`
checks that the checked-out branch is uniquely owned by the current worktree.
"""
