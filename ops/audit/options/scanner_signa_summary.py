#!/usr/bin/env python3
"""scanner-signa-summary -- READ-ONLY audit of today's Signa usage by the options scanner.

Forced-command name: scanner-signa-summary   (no arguments; argv ignored)

EVIDENCE ONLY
  This script does not issue an authoritative PASS/FAIL verdict for #989
  (Signa request control). It prints counts and flags. Humans or agents read
  that evidence and decide. components_signa_all_zero=yes|NO is a fact about
  the rows, not a verdict. There is no verdict line.

PURPOSE
  For today (America/New_York calendar day), from scans.raw_json /
  scans.components_json: signa_symbol vs ticker (flags e.g. XOM->XO),
  signa_client_cached and provider signa_cached true/false/null/missing counts,
  signa_error histogram (http_429, account_backoff_active, ...), first RTH
  (>= 09:30 ET) Signa retrieval, components.signa histogram (expected all 0),
  estimated Signa calls per symbol; plus today's options_signa_context and
  signa_snapshots row counts and status/error histograms.

READ-ONLY GUARANTEES
  sqlite3 'file:<db>?mode=ro' URI + PRAGMA query_only=ON; never writes; creates
  no files; reads no env; never prints signa_raw_payload / payload_json bodies,
  API keys, URLs or account ids -- only whitelisted scalar fields, truncated and
  secret-masked. WAL DB without sidecars => ERROR, exit 3,
  RESULT_INCOMPLETE=true (ALLOW_IMMUTABLE_FALLBACK is False). Missing or
  unreadable DB or missing `scans` => ERROR line + non-zero exit. Missing
  Signa tables are reported, not fatal.

FIELDS PRINTED
  SIGNA_SYMBOL        ticker signa_symbol rows
  SIGNA_MISMATCH      ticker signa_symbol rows first_et last_et
  CLIENT_CACHED / PROVIDER_CACHED   true false null missing other
  SIGNA_ERROR         value rows tickers
  FIRST_RTH_SIGNA     id time_et ticker signa_retrieved_at_et client_cached
                      provider_cached error   (+ FIRST_RTH_PROVIDER_FETCH, FIRST_429)
  COMPONENT_SIGNA     value rows ; components_signa_all_zero=yes|NO ; NONZERO rows
  SIGNA_CALLS         ticker rows distinct_retrieved_at est_provider_requests
                      client_cached local_short_circuit http_429 first/last_et
  SIGNA_CONTEXT_*     options_signa_context today: status, source, endpoint,
                      payload error/http_status/cached/backoff_active/request_ok,
                      per ticker
  SIGNA_SNAPSHOT_*    signa_snapshots today (by retrieved_at): status,
                      http_status, endpoint, symbol
...
# signa_error values produced locally by sources/signa_client.py without an
# HTTP request to the provider.
LOCAL_SHORT_CIRCUIT = ("account_backoff_active", "missing_api_key", "missing_symbol")


def _flag_counts(tag, rows, alias):
    hist = collections.Counter()
    for r in rows:
        kind = r[alias]
        if kind is None:
            hist["missing"] += 1
        elif kind in ("true", "false", "null"):
            hist[kind] += 1
        else:
            hist["other_" + str(kind)] += 1
    out("%s true=%d false=%d null=%d missing=%d%s" % (
        tag, hist["true"], hist["false"], hist["null"], hist["missing"],
        "".join(" %s=%d" % (k, v) for k, v in sorted(hist.items()) if k.startswith("other_"))))


def _scan_section(conn, start, end):
    jf = [("signa_symbol", "raw_json", "$.signa_symbol", "value"),
          ("client_cached", "raw_json", "$.signa_client_cached", "type"),
          ("provider_cached", "raw_json", "$.signa_cached", "type"),
          ("signa_error", "raw_json", "$.signa_error", "value"),
          ("retrieved_at", "raw_json", "$.signa_retrieved_at", "value"),
          ("v2_error", "raw_json", "$.signa_v2_error", "value"),
          ("comp_signa", "components_json", "$.signa", "value"),
          ("comp_signa_type", "components_json", "$.signa", "type")]
    rows, stats = select_window(conn, "scans", "timestamp", ["id", "source", "ticker"], jf, start, end)
    stats_line("SCANS_TODAY", stats)

    out("## section=signa_symbol")
    pairs = collections.Counter((r["ticker"], r["signa_symbol"]) for r in rows)
    for (ticker, sym), count in sorted(pairs.items(), key=lambda kv: (str(kv[0][0]), str(kv[0][1])))[:MAX_ROWS]:
        emit("SIGNA_SYMBOL", ("ticker", ticker), ("signa_symbol", sym), ("rows", count))
    truncated_note("signa_symbol", len(pairs), min(len(pairs), MAX_ROWS))
    mism = collections.OrderedDict()
    for r in rows:
        if r["signa_symbol"] is not None and str(r["signa_symbol"]) != str(r["ticker"]):
            m = mism.setdefault((r["ticker"], r["signa_symbol"]), [0, r["_ts"], r["_ts"]])
            m[0] += 1
            m[2] = r["_ts"]
    for (ticker, sym), (count, first, last) in list(mism.items())[:MAX_ROWS]:
        emit("SIGNA_MISMATCH", ("ticker", ticker), ("signa_symbol", sym), ("rows", count),
             ("first_et", fmt_et(first)), ("last_et", fmt_et(last)))
    out("TOTAL signa_symbol_mismatch_pairs=%d rows=%d signa_symbol_missing_rows=%d" % (
        len(mism), sum(v[0] for v in mism.values()), sum(1 for r in rows if r["signa_symbol"] is None)))

    out("## section=cached_flags")
    _flag_counts("CLIENT_CACHED signa_client_cached", rows, "client_cached")
    _flag_counts("PROVIDER_CACHED signa_cached", rows, "provider_cached")

    out("## section=signa_error")
    errs = collections.OrderedDict()
    for r in rows:
        key = "<none>" if r["signa_error"] in (None, "") else str(r["signa_error"])
        errs.setdefault(key, collections.Counter())[r["ticker"]] += 1
    for key, tickers in sorted(errs.items(), key=lambda kv: (-sum(kv[1].values()), kv[0]))[:MAX_ROWS]:
        emit("SIGNA_ERROR", ("value", key, 120), ("rows", sum(tickers.values())),
             ("tickers", fmt_counts(tickers), 400))
    truncated_note("signa_error", len(errs), min(len(errs), MAX_ROWS))
    out("TOTAL signa_error_http_429=%d signa_error_account_backoff_active=%d signa_v2_error_rows=%d" % (
        sum(errs.get("http_429", {}).values()), sum(errs.get("account_backoff_active", {}).values()),
        sum(1 for r in rows if r["v2_error"] not in (None, ""))))

    out("## section=first_rth")
    def first(pred):
        for r in rows:
            et = r["_ts"].astimezone(ET)
            if (et.hour, et.minute) >= (9, 30) and pred(r):
                return r
        return None
    for tag, pred in (
            ("FIRST_RTH_SIGNA", lambda r: r["retrieved_at"] not in (None, "")),
            ("FIRST_RTH_PROVIDER_FETCH", lambda r: r["retrieved_at"] not in (None, "")
             and r["client_cached"] != "true" and r["signa_error"] in (None, "")),
            ("FIRST_429", lambda r: r["signa_error"] == "http_429")):
        r = first(pred)
        if r is None:
            out("%s none" % tag)
            continue
        emit(tag, ("id", r["id"]), ("time_et", fmt_et(r["_ts"])), ("ticker", r["ticker"]),
             ("signa_retrieved_at_et", fmt_et(parse_ts(r["retrieved_at"]))),
             ("client_cached", r["client_cached"] or "missing"),
             ("provider_cached", r["provider_cached"] or "missing"), ("error", r["signa_error"]))

    out("## section=components_signa")
    comp = collections.Counter()
    nonzero = []
    for r in rows:
        if r["comp_signa_type"] is None:
            comp["<missing>"] += 1
            nonzero.append(r)
            continue
        value = num(r["comp_signa"])
        comp[repr(value) if value is not None else "<%s>" % r["comp_signa_type"]] += 1
        if value != 0:
            nonzero.append(r)
    for key, count in sorted(comp.items()):
        emit("COMPONENT_SIGNA", ("value", key), ("rows", count))
    out("TOTAL components_signa_all_zero=%s nonzero_or_missing_rows=%d" % (
        "yes" if rows and not nonzero else ("NO" if nonzero else "n/a_no_rows"), len(nonzero)))
    for r in nonzero[:50]:
        emit("NONZERO_COMPONENT_SIGNA", ("id", r["id"]), ("time_et", fmt_et(r["_ts"])),
             ("ticker", r["ticker"]), ("value", r["comp_signa"]), ("type", r["comp_signa_type"]))
    truncated_note("nonzero_component_signa", len(nonzero), min(len(nonzero), 50))

    out("## section=signa_calls (est_provider_requests = distinct signa_retrieved_at among rows "
        "that were not client_cached and had no local short-circuit error)")
    per = collections.OrderedDict()
    for r in rows:
        p = per.setdefault(r["ticker"], {"rows": 0, "ret": set(), "req": set(), "cc": 0, "local": 0,
                                         "e429": 0, "first": None, "last": None})
        p["rows"] += 1
        if r["retrieved_at"] not in (None, ""):
            p["ret"].add(r["retrieved_at"])
        if r["client_cached"] == "true":
            p["cc"] += 1
        elif r["signa_error"] in LOCAL_SHORT_CIRCUIT:
            p["local"] += 1
        elif r["retrieved_at"] not in (None, ""):
            p["req"].add(r["retrieved_at"])
            stamp = parse_ts(r["retrieved_at"])
            if stamp is not None:
                p["first"] = stamp if p["first"] is None else min(p["first"], stamp)
                p["last"] = stamp if p["last"] is None else max(p["last"], stamp)
        if r["signa_error"] == "http_429":
            p["e429"] += 1
    for ticker in sorted(per, key=str)[:MAX_ROWS]:
        p = per[ticker]
        emit("SIGNA_CALLS", ("ticker", ticker), ("rows", p["rows"]), ("distinct_retrieved_at", len(p["ret"])),
             ("est_provider_requests", len(p["req"])), ("client_cached", p["cc"]),
             ("local_short_circuit", p["local"]), ("http_429", p["e429"]),
             ("first_request_et", fmt_et(p["first"])), ("last_request_et", fmt_et(p["last"])))
    truncated_note("signa_calls", len(per), min(len(per), MAX_ROWS))
    out("TOTAL est_provider_requests_today=%d distinct_retrieved_at_today=%d" % (
        sum(len(p["req"]) for p in per.values()), sum(len(p["ret"]) for p in per.values())))


def _hist_lines(tag, rows, key):
    hist = collections.Counter("<null>" if r[key] is None else str(r[key]) for r in rows)
    for value, count in sorted(hist.items(), key=lambda kv: (-kv[1], kv[0]))[:60]:
        emit(tag, (key, value, 120), ("rows", count))
    truncated_note(tag.lower(), len(hist), min(len(hist), 60))


def _context_section(conn, tables, start, end):
    out("## section=options_signa_context (today by timestamp)")
    if "options_signa_context" not in tables:
        out("SIGNA_CONTEXT table_missing=options_signa_context")
        return
    jf = [("error", "payload_json", "$.error", "value"),
          ("http_status", "payload_json", "$.http_status", "value"),
          ("cached", "payload_json", "$.cached", "type"),
          ("backoff_active", "payload_json", "$.backoff_active", "type"),
          ("request_ok", "payload_json", "$.request_ok", "type")]
    rows, stats = select_window(conn, "options_signa_context", "timestamp",
                                ["id", "ticker", "source", "endpoint", "status"], jf, start, end)
    stats_line("SIGNA_CONTEXT_TODAY", stats)
    for key in ("status", "source", "endpoint", "error", "http_status", "cached", "backoff_active", "request_ok"):
        _hist_lines("SIGNA_CONTEXT_" + key.upper(), rows, key)
    per = collections.Counter(r["ticker"] for r in rows)
    out("SIGNA_CONTEXT_TICKERS %s" % safe(fmt_counts(per, 100), 2000))


def _snapshot_section(conn, tables, start, end):
    out("## section=signa_snapshots (today by retrieved_at)")
    if "signa_snapshots" not in tables:
        out("SIGNA_SNAPSHOT table_missing=signa_snapshots")
        return
    rows, stats = select_window(conn, "signa_snapshots", "retrieved_at",
                                ["id", "symbol", "endpoint", "status", "http_status", "source"], [], start, end)
    stats_line("SIGNA_SNAPSHOT_TODAY", stats)
    for key in ("status", "http_status", "endpoint", "source"):
        _hist_lines("SIGNA_SNAPSHOT_" + key.upper(), rows, key)
    per = collections.Counter(r["symbol"] for r in rows)
    out("SIGNA_SNAPSHOT_SYMBOLS %s" % safe(fmt_counts(per, 100), 2000))
    if rows:
        out("SIGNA_SNAPSHOT first_et=%s last_et=%s" % (fmt_et(rows[0]["_ts"]), fmt_et(rows[-1]["_ts"])))
...
def main(now=None):
    ...
    _scan_section(conn, start, end)
    _context_section(conn, tables, start, end)
    _snapshot_section(conn, tables, start, end)
