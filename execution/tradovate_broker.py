...
class TradovateBroker(BrokerInterface):
    ...
    # auto-flattened every entry to $0. Poll over a wider window and corroborate
    # via /order/list before failing closed. A genuinely DEAD status still fails
        ...
        for attempt in range(max(1, retries)):
            try:
                ...
                if status in self._DEAD_ORDER_STATUSES:
                    logger.error(
                        "bracket child id=%s DEAD status=%s reason=%s",
                        order_id, status,
