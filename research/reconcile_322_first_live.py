...
Ported verbatim (diffed against `git show fa7babd:research/reconcile_322_first_live.py`
on the abandoned, never-PR'd `codex/4hr-reconciliation` branch) as part of the
2026-07-26 salvage-and-independently-verify effort.

NOTE (2026-07-26): the original external human-study manual-entries CSV this
module was designed to reconcile against was never committed anywhere in this
repository's git history (searched all commits/branches; none found). This
module is ported anyway for completeness and potential future use, but it
cannot currently be exercised end-to-end against real external-study data.
Detector correctness for this evidence study is instead established via the
full ported unit-test suite (tests/test_detector_322_first_live.py,
tests/test_reconcile_322_first_live.py) plus a live run cross-checked against
the documented 32-signal cohort and the named 2024-08-30 SHORT spot-check. See
docs/strategy-rules/60M_322_EXPANDED_EVIDENCE_2026-07-26.md.
"""
