...
0a / 0b  OHLC parity, forward-source bars (Polygon, prereg §9.4) vs a frozen
         corpus, on the prereg overlap dates: PASS iff >= 99.5% of overlapping
         bars match O/H/L/C within 1 tick AND no RTH gap of > 2 consecutive
         bars in the source series that the corpus does not also have.
0c       Candidate parity: each frozen #915 family adapter over the rebuilt
         ...
         §9.4 adds one pre-declared explanation class (holiday day boundary,
         see ``holiday_boundary_explanations``).
0d       4HR-audit lineage: the 4HR, 3-2-2, Miyagi and Daily adapters over
         ...
         plus (§9.4) the 3-2-2 15m detector corpus replay_polygon vs
         replay_corpus_v1_market_condition_fixed and the Miyagi fill corpus
         replay_polygon_5m vs replay_corpus_v1_5m.
...
def ohlc_parity(
    box: LoadedBars,  # forward-source bars (Polygon since amendment 1)
    corpus_root: Path,
    start: date,
    end: date,
    *,
    ...
    return {
        ...
        "corpus_rth_bars_missing_in_source": missing_rth,
        "rth_gap_runs_le_2_bars": short_gaps,
        "rth_gap_violations_gt_2_bars": violations,
        "source_bars_not_in_corpus": len(box_only),
        "source_bars_not_in_corpus_examples": [_iso(t) for t in box_only[:20]],
        "verdict": verdict,
    }
...
# ─── §9.4 pre-declared 0c explanation class: holiday day boundary ────────────
#
# The current derive_candles uses the holiday-aware CME trade-date identity
# (#775, context/cme_trading_day.py), which post-dates the frozen corpora. On the
# CME days just after an exchange holiday / early close the day-boundary fields
# differ. A 0c mismatch is explained by this class only if (a) every timestamp
# it carries lies on one of these UTC days, (b) on each of those days the
# corpora differ in day-boundary fields only (O/H/L/C identical), and (c) across
# the whole overlap no non-boundary field differs anywhere.

HOLIDAY_BOUNDARY_DAYS = frozenset({
    "2026-06-21", "2026-06-22", "2026-06-23",  # after Juneteenth (Fri 06-19)
    "2026-07-05", "2026-07-06", "2026-07-07",  # after July 3/4
})
BOUNDARY_FIELD_PREFIXES = ("previous_day_", "daily_", "ftfc_")
BOUNDARY_FIELDS = frozenset({"vwap", "hod", "lod", "price_vs_pdh", "price_vs_pdl", "price_vs_vwap"})
HOLIDAY_CLASS_TEXT = (
    "PREDECLARED_HOLIDAY_BOUNDARY (prereg §9.4): day-boundary fields differ after "
    "an exchange holiday because derive_candles now uses the holiday-aware trade "
    "date (#775); O/H/L/C identical"
)


def is_boundary_field(name: str) -> bool:
    return name in BOUNDARY_FIELDS or name.startswith(BOUNDARY_FIELD_PREFIXES)


def corpus_field_diffs(frozen_root: Path, rebuilt_root: Path, start: date, end: date) -> dict[str, list[str]]:
    """UTC day -> sorted fields that differ on shared timestamps (plus
    ``__missing__`` when a timestamp exists on one side only)."""
    ...
    def load(root: Path) -> dict[str, dict]:
        ...
        for path in sorted((Path(root) / "MNQ").glob("MNQ_*.jsonl")):
            ...
            for line in path.read_text().splitlines():
                if line.strip():
                    r = json.loads(line)
                    ts = datetime.fromisoformat(r["timestamp"].replace("Z", "+00:00")).astimezone(UTC).isoformat()
                    out[ts] = r
        return out

    a, b = load(frozen_root), load(rebuilt_root)
    diffs: dict[str, set] = defaultdict(set)
    for ts in set(a) | set(b):
        if ts not in a or ts not in b:
            diffs[ts[:10]].add("__missing__")
            continue
        for k in set(a[ts]) & set(b[ts]):
            x, y = a[ts][k], b[ts][k]
            if isinstance(x, (int, float)) and isinstance(y, (int, float)) and not isinstance(x, bool):
                ...
                diffs[ts[:10]].add(k)
    return {d: sorted(v) for d, v in sorted(diffs.items())}


def _mismatch_utc_days(m: dict) -> set[str]:
    stamps: list[str] = []
    if m.get("ts"):
        stamps.append(m["ts"])
    for v in m.values():
        if isinstance(v, dict):
            for k in ("signal_ts", "eligible_fill_ts"):
                if v.get(k):
                    stamps.append(v[k])
    days = set()
    for s in stamps:
        try:
            days.add(datetime.fromisoformat(str(s).replace("Z", "+00:00")).astimezone(UTC).date().isoformat())
        except ValueError:
            return set()
    return days


def holiday_boundary_explanations(mismatches: list[dict], field_diffs: dict[str, list[str]]) -> dict[str, str]:
    """Auto-explanations for the §9.4 class (empty if condition (c) fails)."""
    if any(not is_boundary_field(f) for fields in field_diffs.values() for f in fields):
        return {}
    out = {}
    for m in mismatches:
        if m.get("kind") == "ADAPTER_FAILED_CLOSED":
            continue
        days = _mismatch_utc_days(m)
        if days and days <= HOLIDAY_BOUNDARY_DAYS and all(field_diffs.get(d) for d in days):
            out[m["id"]] = HOLIDAY_CLASS_TEXT
    return out
...
def summarize_text(report: dict) -> str:
    ...
    for k in ("0a", "0b"):
        ...
        lines.append(
            ...
            f"{r['corpus_rth_bars_missing_in_source']} corpus RTH bars missing in source; "
    ...
    for k in ("0c", "0d"):
        ...
        if r.get("predeclared_holiday_class_applied"):
            lines.append(f"{k} pre-declared holiday-boundary class explained {r['predeclared_holiday_class_applied']}")
        lines.append(
    ...
    for k, v in (report.get("supplementary") or {}).items():
        if not isinstance(v, dict) or k == "corpus_field_diffs_by_utc_day":
            continue
        if "identical" in v:
