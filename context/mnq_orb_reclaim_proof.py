"""MNQ orb_reclaim proof mode — Stage 2 of the ORB no-fill investigation.

Scoped narrowly: MNQ + orb_reclaim only. This module never runs for any other
instrument or strategy — MNQ range_break_close, MES, and the generic
RANGE_BOUND/require_trending_condition gate are all untouched by anything here.

Modes (config.mnq_orb_reclaim_proof_mode / MNQ_ORB_RECLAIM_PROOF_MODE env,
...
direction) — a repeated orb_reclaim alert riding the same ORB move does not
...
def mnq_orb_reclaim_proof_mode(cfg=None) -> str:
    ...
    raw = getattr(cfg, "mnq_orb_reclaim_proof_mode", None)
    if raw is None:
        raw = os.getenv("MNQ_ORB_RECLAIM_PROOF_MODE", DEFAULT_MODE)
    raw = str(raw or DEFAULT_MODE).strip().lower()
    return raw if raw in VALID_MODES else DEFAULT_MODE


def is_mnq_orb_reclaim_candidate(instrument: Optional[str], strategy: Optional[str]) -> bool:
    root = (instrument or "").upper().replace("1!", "")
    return root == "MNQ" and strategy == "orb_reclaim"


def _campaign_path(log_dir: str, for_date=None) -> Path:
    d = for_date or date.today()
    return Path(log_dir) / f"mnq_orb_reclaim_proof_campaigns_{d.isoformat()}.json"
...
class ProofDecision:
    ...
    - observe_only            -> suppress=False, apply_override=False. The
      EXISTING orb_reclaim decision (TRADE_INTENT -> risk -> broker, current
...
def evaluate_mnq_orb_reclaim_proof(
    *, cfg, log_dir: str, orb_high, orb_low, direction, for_date=None
    ...
    mode = mnq_orb_reclaim_proof_mode(cfg)
